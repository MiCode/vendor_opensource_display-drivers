...
int mi_dsi_panel_init(struct dsi_panel *panel)
{
	...
	if ((mi_get_panel_id_by_dsi_panel(panel) == N3_PANEL_PA) ||
		...
		(mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PB) ||
		...
		(mi_get_panel_id_by_dsi_panel(panel) == O16U_PANEL_PC) ||
		...
		(mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PA) ||
		(mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PC) ||
		(mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PB)) {
...
bool is_backlight_set_skip(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (mi_cfg->in_fod_calibration) {
		...
		if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N3_PANEL_PA ||
			...
			mi_get_panel_id(panel->mi_cfg.mi_panel_id) == N8_PANEL_PB ||
...
int mi_dsi_send_lhbm_sync_with_te_locked(struct dsi_panel *panel,
			struct disp_feature_ctl *ctl)
{
	int rc = 0;
	...
	case LOCAL_HBM_HLPM_WHITE_110NIT:
		...
		break;
	default:
		mi_cfg->cur_lhbm_on_state = FEATURE_OFF;
		DISP_DEBUG("set cur_lhbm_on_state off\n");
		DISP_DEBUG("local hbm off cmd should send out sync with te");
		break;
	}
	return rc;
}
...
static int mi_dsi_panel_update_lhbm_param_P16U(struct dsi_panel * panel,
		enum dsi_cmd_set_type type, int bl_lvl)
{
	struct dsi_display_mode_priv_info *priv_info = NULL;
	struct dsi_cmd_update_info *info = NULL;
	...
	else if (bl_lvl < 0x18E0) bl_index = 1;
	else if (bl_lvl < 0x1FFF) bl_index = 2;
	else if (bl_lvl < 0x2557) bl_index = 3;
	else if (bl_lvl < 0x2B11) bl_index = 4;
	else if (bl_lvl < 0x2B74) bl_index = 5;
	else if (bl_lvl < 0x2BD7) bl_index = 6;
	else if (bl_lvl < 0x2C3A) bl_index = 7;
	else if (bl_lvl < 0x2C9D) bl_index = 8;
	else if (bl_lvl < 0x2D00) bl_index = 9;
	else if (bl_lvl < 0x2D63) bl_index = 10;
	else if (bl_lvl < 0x2DC5) bl_index = 11;
	else if (bl_lvl < 0x2E28) bl_index = 12;
	else if (bl_lvl < 0x2E8B) bl_index = 13;
	else if (bl_lvl < 0x2EEE) bl_index = 14;
	else if (bl_lvl < 0x2F51) bl_index = 15;
	else if (bl_lvl < 0x2FB4) bl_index = 16;
	else if (bl_lvl < 0x3017) bl_index = 17;
	else if (bl_lvl < 0x307A) bl_index = 18;
	else if (bl_lvl < 0x30DC) bl_index = 19;
	else if (bl_lvl < 0x313F) bl_index = 20;
	else if (bl_lvl < 0x31A2) bl_index = 21;
	else if (bl_lvl < 0x3205) bl_index = 22;
	else if (bl_lvl < 0x3268) bl_index = 23;
	else if (bl_lvl < 0x32CB) bl_index = 24;
	else if (bl_lvl < 0x332E) bl_index = 25;
	else if (bl_lvl < 0x3390) bl_index = 26;
	else if (bl_lvl <= 0x3FFF) bl_index = 27;
	...
	switch (type) {
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[0],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_high_ratio_pa[bl_index])/1000;
		...
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[6],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_low_ratio_pa[bl_index])/1000;
				d1_reg_buf[i-1] = (tmp >> 8) & 0xFF;
				d1_reg_buf[i] = tmp & 0xFF;
			}
			break;
...
static int mi_dsi_panel_update_lhbm_param_P16U_PB(struct dsi_panel * panel,
		enum dsi_cmd_set_type type, int bl_lvl)
{
	struct dsi_display_mode_priv_info *priv_info = NULL;
	struct dsi_cmd_update_info *info = NULL;
	...
	else if (bl_lvl <= 0x065B) bl_index = 1;
	else if (bl_lvl <= 0x087E) bl_index = 2;
	else if (bl_lvl <= 0x0AA1) bl_index = 3;
	else if (bl_lvl <= 0x0EE7) bl_index = 4;
	else if (bl_lvl <= 0x132D) bl_index = 5;
	else if (bl_lvl <= 0x1773) bl_index = 6;
	else if (bl_lvl <= 0x1BB9) bl_index = 7;
	else if (bl_lvl <= 0x1FFF) bl_index = 8;
	else if (bl_lvl <= 0x22AB) bl_index = 9;
	else if (bl_lvl <= 0x2557) bl_index = 10;
	else if (bl_lvl <= 0x2803) bl_index = 11;
	else if (bl_lvl <= 0x2AAE) bl_index = 12;
	else if (bl_lvl <= 0x2B5D) bl_index = 13;
	else if (bl_lvl <= 0x2C0C) bl_index = 14;
	else if (bl_lvl <= 0x2CBB) bl_index = 15;
	else if (bl_lvl <= 0x2D6A) bl_index = 16;
	else if (bl_lvl <= 0x2E19) bl_index = 17;
	else if (bl_lvl <= 0x2EC8) bl_index = 18;
	else if (bl_lvl <= 0x2F77) bl_index = 19;
	else if (bl_lvl <= 0x3026) bl_index = 20;
	else if (bl_lvl <= 0x30D5) bl_index = 21;
	else if (bl_lvl <= 0x3184) bl_index = 22;
	else if (bl_lvl <= 0x3233) bl_index = 23;
	else if (bl_lvl <= 0x32E2) bl_index = 24;
	...
	switch (type) {
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[0],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_high_ratio_pb[(i-1)/2][bl_index])/1000;
		...
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[6],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_low_ratio_pb[(i-1)/2][bl_index])/1000;
				d1_reg_buf[i-1] = (tmp >> 8) & 0x3F;
				d1_reg_buf[i] = tmp & 0xFF;
			}
			break;
...
static int mi_dsi_panel_update_lhbm_param_P16U_PC(struct dsi_panel * panel,
		enum dsi_cmd_set_type type, int bl_lvl)
{
	struct dsi_display_mode_priv_info *priv_info = NULL;
	struct dsi_cmd_update_info *info = NULL;
	...
	if (0x0 <= bl_lvl && bl_lvl <= 0x000F) bl_index = 0;
	...
	switch (type) {
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[0],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_high_ratio_pc[(i-1)/2][bl_index])/1000;
				d1_reg_buf[i-1] = (tmp >> 8) & 0xFF;
				d1_reg_buf[i] = tmp & 0xFF;
			}
			break;
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT:
			...
			memcpy(&d1_reg_buf[0], &panel->mi_cfg.lhbm_rgb_param[6],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((d1_reg_buf[i-1]) << 8) | d1_reg_buf[i])*4*(lhbm_p16u_white_low_ratio_pc[(i-1)/2][bl_index])/1000;
				d1_reg_buf[i-1] = (tmp >> 8) & 0xFF;
				d1_reg_buf[i] = tmp & 0xFF;
			}
			break;
...
int mi_dsi_panel_nova_csc_by_temper_comp_locked(struct dsi_panel *panel, int temp_val)
{
	...
	mi_cfg = &panel->mi_cfg;
	if (mi_get_panel_id_by_dsi_panel(panel) != P16U_PANEL_PA && mi_get_panel_id_by_dsi_panel(panel) != P16U_PANEL_PB && mi_get_panel_id_by_dsi_panel(panel) != P16U_PANEL_PC) {
		return rc;
	}

	if (temp_val < 26) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_OFF_MODE);
	} else if (temp_val >= 26 && temp_val < 28) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_26_28_MODE);
	} else if (temp_val >= 28 && temp_val < 30) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_28_30_MODE);
	} else if (temp_val >= 30 && temp_val < 32) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_30_32_MODE);
	} else if (temp_val >= 32 && temp_val < 34) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_32_34_MODE);
	} else if (temp_val >= 34 && temp_val < 36) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_34_36_MODE);
	} else if (temp_val >= 36 && temp_val < 38) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_36_38_MODE);
	} else if (temp_val >= 38 && temp_val < 40) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_38_40_MODE);
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_40_MODE);
	}
...
int mi_dsi_panel_update_lhbm_param_N11U(struct dsi_panel * panel,
		enum dsi_cmd_set_type type, int flat_mode, int bl_lvl)
{
	int rc = 0;
	struct dsi_display_mode *cur_mode = NULL;
		...
		struct disp_feature_ctl *ctl)
{
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		case O16U_PANEL_PB:
			...
			break;
		case P16U_PANEL_PA:
			mi_dsi_panel_update_lhbm_param_P16U(panel, lhbm_type, update_bl_lvl);
			break;
		case P16U_PANEL_PB:
			mi_dsi_panel_update_lhbm_param_P16U_PB(panel, lhbm_type, update_bl_lvl);
			break;
		case P16U_PANEL_PC:
			mi_dsi_panel_update_lhbm_param_P16U_PC(panel, lhbm_type, update_bl_lvl);
			break;
		default:
...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_FLAT_MODE:
		if (!mi_cfg->flat_sync_te) {
			if (ctl->feature_val == FEATURE_ON) {
				...
				if (mi_get_panel_id_by_dsi_panel(panel) == N2_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == N1_PANEL_PA ||
					mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_PB || mi_get_panel_id_by_dsi_panel(panel) == N8_PANEL_SA)
	...
	case DISP_FEATURE_LOCAL_HBM:
		if (!mi_cfg->lhbm_off_sync_te) {
			...
			rc = mi_dsi_send_lhbm_sync_with_te_locked(panel, ctl);
		}	
	...
	case DISP_FEATURE_DBI:
		if (mi_get_panel_id_by_dsi_panel(panel) == N11U_PANEL_PA ||
			...
			mi_cfg->feature_val[DISP_FEATURE_DBI] = ctl->feature_val;
		} else if (mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PA ||
				mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PC ||
				mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PB) {
			mi_dsi_panel_nova_csc_by_temper_comp_locked(panel, ctl->feature_val);
			mi_cfg->feature_val[DISP_FEATURE_DBI] = ctl->feature_val;
			DISP_INFO("[%s] csc setting, rc=[%d]\n", panel->type, ctl->feature_val);
		} else {
			DISP_INFO("by temp gray_level:%d\n", ctl->feature_val);
...
int mi_dsi_panel_set_dbi_by_temp_bl_N8(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (mi_get_panel_id_by_dsi_panel(panel) != N8_PANEL_PA && mi_get_panel_id_by_dsi_panel(panel) != N8_PANEL_PB && mi_get_panel_id_by_dsi_panel(panel) != N8_PANEL_SA) {
		return 0;
	}
