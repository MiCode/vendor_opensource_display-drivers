		...
		enum dsi_cmd_set_type type, int bl_lvl)
{
	...
	u32 cmd_update_index = 0;
	u32 cmd_update_count = 0;
	u8 update_DF_reg[6] = {0};
	u8 update_DF_reg_green_500nit[2] = {0};
	int type_index = 0;
	int bl_index = 0;
	u8 lhbm_O9_WHITE_1000NIT_DF_reg[17][6] = {
		{0x3F,0xFC,0x3F,0xFC,0x3F,0xFC}, //0~2CBB
		{0x3E,0xFC,0x3E,0xFC,0x3E,0xFC}, //>=2CBC
		{0x39,0x8C,0x39,0x8C,0x39,0x8C}, //>=2DDF
		{0x36,0x5C,0x36,0x5C,0x36,0x5C}, //>=2EF3
		{0x33,0x4C,0x33,0x4C,0x33,0x4C}, //>=3007
		{0x30,0x4C,0x30,0x4C,0x30,0x4C}, //>=311B
		{0x2B,0xFC,0x2B,0xFC,0x2B,0xFC}, //>=322F
		{0x29,0x3C,0x29,0x3C,0x29,0x3C}, //>=3343
		{0x26,0x8C,0x26,0x8C,0x26,0x8C}, //>=3457
		{0x23,0xFC,0x23,0xFC,0x23,0xFC}, //>=356B
		{0x21,0x9C,0x21,0x9C,0x21,0x9C}, //>=367F
		{0x3E,0x4B,0x3E,0x4B,0x3E,0x4B}, //>=3793
		{0x39,0xBB,0x39,0xBB,0x39,0xBB}, //>=38A7
		{0x35,0x6B,0x35,0x6B,0x35,0x6B}, //>=39BB
		{0x31,0x3B,0x31,0x3B,0x31,0x3B}, //>=3ACF
		{0x2D,0x2B,0x2D,0x2B,0x2D,0x2B}, //>=3BE3
		{0x28,0xCB,0x28,0xCB,0x28,0xCB}  //>=3CF3
	};
	u8 lhbm_O9_WHITE_110NIT_DF_reg[17][6] = {
		{0x32,0xFA,0x32,0xFA,0x32,0xFA}, //0~2CBB
		{0x2F,0xFA,0x2F,0xFA,0x2F,0xFA}, //>=2CBC
		{0x29,0xFA,0x29,0xFA,0x29,0xFA}, //>=2DDF
		{0x26,0xFA,0x26,0xFA,0x26,0xFA}, //>=2EF3
		{0x22,0xFA,0x22,0xFA,0x22,0xFA}, //>=3007
		{0x3E,0xC9,0x3E,0xC9,0x3E,0xC9}, //>=311B
		{0x38,0xC9,0x38,0xC9,0x38,0xC9}, //>=322F
		{0x33,0xC9,0x33,0xC9,0x33,0xC9}, //>=3343
		{0x2E,0xC9,0x2E,0xC9,0x2E,0xC9}, //>=3457
		{0x2A,0xC9,0x2A,0xC9,0x2A,0xC9}, //>=356B
		{0x27,0xC9,0x27,0xC9,0x27,0xC9}, //>=367F
		{0x23,0xC9,0x23,0xC9,0x23,0xC9}, //>=3793
		{0x21,0xC9,0x21,0xC9,0x21,0xC9}, //>=38A7
		{0x3C,0xB8,0x3C,0xB8,0x3C,0xB8}, //>=39BB
		{0x38,0xB8,0x38,0xB8,0x38,0xB8}, //>=3ACF
		{0x37,0xB8,0x37,0xB8,0x37,0xB8}, //>=3BE3
		{0x32,0x58,0x32,0x58,0x32,0x58}, //>=3CF3
	};
	u8 lhbm_O9_GREEN_500NIT_DF_reg[17][2] = {
		{0x2C,0x9C}, //0~2CBB
		{0x29,0x7C}, //>=2CBC
		{0x25,0x7C}, //>=2DDF
		{0x22,0x7C}, //>=2EF3
	...
	if (bl_lvl <= 11451) bl_index = 0;
	else if (11451 <= bl_lvl && bl_lvl < 11743) bl_index = 1;
	else if (11743 <= bl_lvl && bl_lvl < 12019) bl_index = 2;
	else if (12019 <= bl_lvl && bl_lvl < 12295) bl_index = 3;
	else if (12295 <= bl_lvl && bl_lvl < 12571) bl_index = 4;
	else if (12571 <= bl_lvl && bl_lvl < 12847) bl_index = 5;
	else if (12847 <= bl_lvl && bl_lvl < 13123) bl_index = 6;
	else if (13123 <= bl_lvl && bl_lvl < 13399) bl_index = 7;
	else if (13399 <= bl_lvl && bl_lvl < 13675) bl_index = 8;
	else if (13675 <= bl_lvl && bl_lvl < 13951) bl_index = 9;
	else if (13951 <= bl_lvl && bl_lvl < 14227) bl_index = 10;
	else if (14227 <= bl_lvl && bl_lvl < 14503) bl_index = 11;
	else if (14503 <= bl_lvl && bl_lvl < 14779) bl_index = 12;
	else if (14779 <= bl_lvl && bl_lvl < 15055) bl_index = 13;
	else if (15055 <= bl_lvl && bl_lvl < 15331) bl_index = 14;
	else if (15331 <= bl_lvl && bl_lvl < 15603) bl_index = 15;
	else if (15603 <= bl_lvl) bl_index = 16;
	switch (type) {
	...
	if(type_index== 0){
		memcpy(update_DF_reg, lhbm_O9_WHITE_1000NIT_DF_reg[bl_index], sizeof(update_DF_reg));
	} else if(type_index == 1){
		memcpy(update_DF_reg, lhbm_O9_WHITE_110NIT_DF_reg[bl_index], sizeof(update_DF_reg));
	} else if(type_index == 2){
		memcpy(update_DF_reg_green_500nit, lhbm_O9_GREEN_500NIT_DF_reg[bl_index], sizeof(update_DF_reg_green_500nit));
	}
	DISP_INFO("[%s] bl_lvl = %d, type_index = %d, bl_index=%d, update_DF_reg[0]= 0x%x, update_DF_reg[1]= 0x%x, update_DF_reg[2]= 0x%x, update_DF_reg[3]= 0x%x, update_DF_reg[4]= 0x%x, update_DF_reg[5]= 0x%x, update_DF_reg_green_500nit[0]= 0x%x, update_DF_reg_green_500nit[1]= 0x%x\n",
		panel->type, bl_lvl, type_index, bl_index, update_DF_reg[0], update_DF_reg[1], update_DF_reg[2], update_DF_reg[3], update_DF_reg[4], update_DF_reg[5], update_DF_reg_green_500nit[0], update_DF_reg_green_500nit[1]);
	cur_mode = panel->cur_mode;
	info = cur_mode->priv_info->cmd_update[cmd_update_index];
	cmd_update_count = cur_mode->priv_info->cmd_update_count[cmd_update_index];
	for (int i = 0; i < cmd_update_count; i++) {
		DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
			panel->type, cmd_set_prop_map[info->type],
			info->mipi_address, info->index, info->length);
			if (info && info->mipi_address == 0xDF && info->length == 6) {
				...
				mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
					update_DF_reg_green_500nit, sizeof(update_DF_reg_green_500nit));
			}
		info++;
	}
	return rc;
		...
		struct disp_feature_ctl *ctl)
{
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (mi_get_panel_id_by_dsi_panel(panel) == O9_PANEL_PA) {
			mi_dsi_panel_update_lhbm_param_O9(panel, lhbm_type, mi_cfg->last_bl_level);
		}
		rc = dsi_panel_tx_cmd_set(panel, lhbm_type, false);
		break;
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
...
void mi_dsi_panel_update_ic_dimming_by_bl(struct dsi_panel *panel, u32 bl_lvl) {
	if(mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_SA){
		if(panel->mi_cfg.last_bl_level > bl_lvl && (panel->mi_cfg.last_bl_level - bl_lvl > 8191))
		{
