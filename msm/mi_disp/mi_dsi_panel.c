...
#define SMEM_SW_DISPLAY_PEAK_OFFSET_TABLE 502
				...
				bool enable)
{
	...
	if (mi_get_panel_id_by_dsi_panel(panel) == P82_PANEL_PA ||
...
int mi_dsi_update_peakoffset_mode_on_cmd(struct dsi_panel *panel,
		struct dsi_display_mode *mode)
{
	...
	if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) != P12X_PANEL_PA) {
		DISP_INFO("panel is not surrported  \n");
		return 0;
	}
	...
	for (i = 0; i < cmd_update_count && i < 36; i++) {
		if (i < 9) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_ON, info,
				mi_cfg->peak_offset_on_60hz[i],
				sizeof(mi_cfg->peak_offset_on_60hz[i]));
		} else if (i < 18) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_ON, info,
				mi_cfg->peak_offset_on_120hz[i - 9],
				sizeof(mi_cfg->peak_offset_on_120hz[i - 9]));
		} else if (i < 27) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_ON, info,
				mi_cfg->peak_offset_on_144hz[i - 18],
				sizeof(mi_cfg->peak_offset_on_144hz[i - 18]));
		} else {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_ON, info,
				mi_cfg->peak_offset_on_165hz[i - 27],
				sizeof(mi_cfg->peak_offset_on_165hz[i - 27]));
		}
		info++;
	}
...
int mi_dsi_update_peakoffset_mode_off_cmd(struct dsi_panel *panel,
		struct dsi_display_mode *mode)
{
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
	struct dsi_cmd_update_info *info = NULL;
	...
	for (i = 0; i < cmd_update_count && i < 36; i++) {
		if (i < 9) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				...
				mi_cfg->peak_offset_off_60hz[i],
				sizeof(mi_cfg->peak_offset_off_60hz[i]));
		} else if (i < 18) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_OFF, info,
				mi_cfg->peak_offset_off_120hz[i - 9],
				sizeof(mi_cfg->peak_offset_off_120hz[i - 9]));
		} else if (i < 27) {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_OFF, info,
				mi_cfg->peak_offset_off_144hz[i - 18],
				sizeof(mi_cfg->peak_offset_off_144hz[i - 18]));
		} else {
			mi_dsi_panel_update_cmd_set(panel, mode,
				DSI_CMD_SET_MI_FLAT_MODE_OFF, info,
				mi_cfg->peak_offset_off_165hz[i - 27],
				sizeof(mi_cfg->peak_offset_off_165hz[i - 27]));
		}
		info++;
	}
...
bool is_backlight_set_skip(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (mi_cfg->in_fod_calibration) {
		...
		if (bl_lvl != 0) {
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O9_PANEL_PA) {
		...
		if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O9_PANEL_PA) {
			mi_cfg->backlight_set_skip_in_lp = true;
			DISP_INFO("[%s] skip set backlight %d due to LP1 on\n", panel->type, bl_lvl);
			return true;
		}
		return false;
			...
			u32 doze_brightness)
{
	...
	if (is_hbm_fod_on(panel)) {
		...
		if (doze_brightness == DOZE_BRIGHTNESS_HBM) {
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O8_PANEL_PA ||
				mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O8_PANEL_SA)
			{
				if (mi_cfg->fp_unlock_success == true)
			...
			enum dsi_cmd_set_type type, int bl_lvl)
{
	...
	case DSI_CMD_SET_MI_DOZE_LBM:
		cmd_update_index = DSI_CMD_SET_MI_DOZE_LBM_UPDATE;
		break;
	case DSI_CMD_SET_MI_APLDIMMING:
		cmd_update_index = DSI_CMD_SET_MI_APLDIMMING_UPDATE;
		break;
	case DSI_CMD_SET_MI_PWM_DEMURA_SWITCH:
		cmd_update_index = DSI_CMD_SET_MI_PWM_DEMURA_SWITCH_UPDATE;
		break;
	case DSI_CMD_SET_MI_DC_DEMURA_SWITCH:
		cmd_update_index = DSI_CMD_SET_MI_DC_DEMURA_SWITCH_UPDATE;
		break;
	default:
...
int mi_dsi_panel_nova_csc_by_temper_comp_locked(struct dsi_panel *panel, int temp_val)
{
	...
	if (mi_get_panel_id_by_dsi_panel(panel) == O9_PANEL_PA) {
		if (temp_val < 28) {
			...
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_28_32_MODE, false);
		} else if (temp_val >= 32 && temp_val < 36) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_32_36_MODE, false);
		} else if (temp_val >= 36 && temp_val < 40) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_36_40_MODE, false);
...
static int mi_dsi_panel_update_lhbm_param_O9(struct dsi_panel * panel,
		enum dsi_cmd_set_type type, int bl_lvl)
{
		struct dsi_display_mode *cur_mode = NULL;
	struct dsi_cmd_update_info *info = NULL;
	...
	u32 cmd_update_count = 0;
	u32 tmp;
	u8 update_D1_reg[6] = {0};
	int bl_index = 0, i = 0, j = 0;
	if (!panel || !panel->cur_mode || !panel->cur_mode->priv_info) {
		DISP_ERROR("invalid params\n");
		return -EINVAL;
	}
	...
	if (0x0 <= bl_lvl && bl_lvl < 0x0C1E) bl_index = 0;
	else if (bl_lvl < 0x0DEC) bl_index = 1;
	else if (bl_lvl < 0x1188) bl_index = 2;
	else if (bl_lvl < 0x1356) bl_index = 3;
	else if (bl_lvl < 0x1524) bl_index = 4;
	else if (bl_lvl < 0x16F2) bl_index = 5;
	else if (bl_lvl < 0x18C0) bl_index = 6;
	else if (bl_lvl < 0x1A8E) bl_index = 7;
	else if (bl_lvl < 0x1C5C) bl_index = 8;
	else if (bl_lvl < 0x1E2A) bl_index = 9;
	else if (bl_lvl < 0x1FFF) bl_index = 10;
	else if (bl_lvl < 0x228F) bl_index = 11;
	else if (bl_lvl < 0x251E) bl_index = 12;
	else if (bl_lvl < 0x27AD) bl_index = 13;
	else if (bl_lvl < 0x2A3C) bl_index = 14;
	else if (bl_lvl < 0x2CCB) bl_index = 15;
	else if (bl_lvl < 0x2E7F) bl_index = 16;
	else if (bl_lvl < 0x2F59) bl_index = 17;
	else if (bl_lvl < 0x3032) bl_index = 18;
	else if (bl_lvl < 0x310C) bl_index = 19;
	else if (bl_lvl < 0x31E6) bl_index = 20;
	else if (bl_lvl < 0x32BF) bl_index = 21;
	else if (bl_lvl < 0x3399) bl_index = 22;
	else if (bl_lvl < 0x3473) bl_index = 23;
	else if (bl_lvl < 0x354C) bl_index = 24;
	else if (bl_lvl < 0x3626) bl_index = 25;
	else if (bl_lvl < 0x3700) bl_index = 26;
	else if (bl_lvl < 0x37D9) bl_index = 27;
	else if (bl_lvl < 0x38B3) bl_index = 28;
	else if (bl_lvl < 0x398D) bl_index = 29;
	else if (bl_lvl < 0x3A66) bl_index = 30;
	else if (bl_lvl < 0x3B40) bl_index = 31;
	else if (bl_lvl < 0x3C1A) bl_index = 32;
	else if (bl_lvl < 0x3CF3) bl_index = 33;
	else if (bl_lvl < 0x3DB0) bl_index = 34;
	else if (bl_lvl >= 0x3DB0) bl_index = 35;
	switch (type) {
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT:
			cmd_update_index = DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT_UPDATE;
			memcpy(&update_D1_reg[0], &panel->mi_cfg.lhbm_rgb_param[0],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((update_D1_reg[i-1]) << 8) | update_D1_reg[i])*4*(lhbm_O9_WHITE_1200NIT_ratio_pa[bl_index][j])/1000;
				update_D1_reg[i-1] = (tmp >> 8) & 0xFF;
				update_D1_reg[i] = tmp & 0xFF;
				j += 1;
			}
			break;
		case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT:
			cmd_update_index = DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_110NIT_UPDATE;
			memcpy(&update_D1_reg[0], &panel->mi_cfg.lhbm_rgb_param[6],sizeof(u8)*6);
			for(i = 1; i < 6; i += 2) {
				tmp = (((update_D1_reg[i-1]) << 8) | update_D1_reg[i])*4*(lhbm_O9_WHITE_200NIT_ratio_pa[bl_index][j])/1000;
				update_D1_reg[i-1] = (tmp >> 8) & 0xFF;
				update_D1_reg[i] = tmp & 0xFF;
				j += 1;
			}
	...
	DISP_INFO("[%s] bl_lvl  = %d(0x%x), bl_index=%d, update_D1_reg[0]= 0x%x, update_D1_reg[1]= 0x%x, update_D1_reg[2]= 0x%x, update_D1_reg[3]= 0x%x, update_D1_reg[4]= 0x%x, update_D1_reg[5]= 0x%x\n",
		panel->type, bl_lvl, bl_lvl, bl_index, update_D1_reg[0], update_D1_reg[1], update_D1_reg[2], update_D1_reg[3], update_D1_reg[4], update_D1_reg[5]);
	cur_mode = panel->cur_mode;
	info = cur_mode->priv_info->cmd_update[cmd_update_index];
	cmd_update_count = cur_mode->priv_info->cmd_update_count[cmd_update_index];
	for (int i = 0; i < cmd_update_count; i++) {
		DISP_DEBUG("[%s] update [%s] mipi_address(0x%02X) index(%d) lenght(%d)\n",
			panel->type, cmd_set_prop_map[info->type],
			info->mipi_address, info->index, info->length);
			if (info && info->mipi_address == 0xD1 && info->length == 6) {
				mi_dsi_panel_update_cmd_set(panel, cur_mode, type, info,
					update_D1_reg, sizeof(update_D1_reg));
			}
		info++;
	}
	return 0;
		...
		struct disp_feature_ctl *ctl)
{
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (mi_get_panel_id_by_dsi_panel(panel) == O9_PANEL_PA) {
			mi_dsi_panel_update_lhbm_param_O9(panel, lhbm_type, update_bl_lvl);
		}
		if (mi_get_panel_id_by_dsi_panel(panel) == O10U_PANEL_PA) {
...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_DOZE_BRIGHTNESS:
		...
		if (dsi_panel_initialized(panel) &&
			...
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PA || mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PB) {
				mi_dsi_update_switch_cmd_O3(panel, DSI_CMD_SET_NOLP_UPDATE,
							DSI_CMD_SET_NOLP);
			}
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == P12X_PANEL_PA) {
				mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_NOLP,
							mi_cfg->last_bl_level);
			}
		...
		if (is_aod_and_panel_initialized(panel) &&
			...
			if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PB) {
				rc = mi_dsi_panel_dbv_demura_compensate(panel, mi_cfg->last_bl_level);
			}
		}
#endif
	...
	case DISP_FEATURE_DBI:
		...
		if(mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_PA ||mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_SA) {
	...
	case DISP_FEATURE_APL_DIMMING:
		if (mi_get_panel_id_by_dsi_panel(panel) == O1_PANEL_PA) {
			mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_MI_APLDIMMING, ctl->feature_val);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_APLDIMMING, false);
			DISP_INFO("[%s] apl_dimming setting, ctl->feature_val [%d] rc=[%d]\n", panel->type, ctl->feature_val, rc);
		}
		break;
	case DISP_FEATURE_LAYER_HAS_AOD:
		DISP_INFO("layer has aod:%d\n", ctl->feature_val);
		panel->mi_cfg.layer_has_aod = ctl->feature_val;
		break;
	case DISP_FEATURE_EARLY_SCREEN_STATUS:
		DISP_INFO("early screen status:%d, disable_ic_dimming :%u\n", ctl->feature_val, mi_cfg->disable_ic_dimming);
		if (ctl->feature_val && !mi_cfg->disable_ic_dimming) {
			panel->mi_cfg.disable_ic_dimming = true;
			panel->mi_cfg.isPreBrightnessOn = true;
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DIMMINGOFF, false);
	...
	case DISP_FEATURE_CABC:
		...
		if (ctl->feature_val == LCD_CABC_OFF) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_OFF, false);
			DISP_INFO("lcd cabc off\n");
		}
		if (ctl->feature_val == LCD_CABC_UI_ON) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_UI_ON, false);
			DISP_INFO("lcd cabc ui on\n");
		}
		if (ctl->feature_val == LCD_CABC_MOVIE_ON) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_MOVIE_ON, false);
			DISP_INFO("lcd cabc movie on\n");
		}
		if (ctl->feature_val == LCD_CABC_STILL_ON) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_STILL_ON, false);
...
int mi_dsi_panel_set_nolp_locked(struct dsi_panel *panel)
{
	...
	if (((mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O1_PANEL_PA) ||
		...
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PB) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O8_PANEL_PA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O8_PANEL_SA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O11U_PANEL_PA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O9_PANEL_PA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == P11_PANEL_PA) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == P11_PANEL_PB) ||
		(mi_get_panel_id(panel->mi_cfg.mi_panel_id) == P12X_PANEL_PA)) &&
		(panel->mi_cfg.last_bl_level != 0)){
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_NOLP, false);
	if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O3_PANEL_PB) {
		mi_dsi_panel_dbv_demura_compensate(panel, update_bl);
	}
exit:
...
int mi_dsi_panel_set_dbi_by_temp_bl_O8(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;
	struct mi_dsi_panel_cfg *mi_cfg;
	...
	if (mi_get_panel_id_by_dsi_panel(panel) != O8_PANEL_PA &&
	    mi_get_panel_id_by_dsi_panel(panel) != O8_PANEL_SA) {
		return 0;
	}
	...
	if (mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_PA) {
		if (temp == 0) {
			...
			if (0x38 > bl_lvl && bl_lvl >= 0x0F) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL0F_MODE;
			} else if (0x7C > bl_lvl && bl_lvl >= 0x38) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL38_MODE;
			} else if (0x105 > bl_lvl && bl_lvl >= 0x7C) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL7C_MODE;
			} else if (0x217 > bl_lvl && bl_lvl >= 0x105) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL105_MODE;
			} else if (0x328 >= bl_lvl && bl_lvl >= 0x217) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL217_MODE;
			} else if (bl_lvl == 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BLEQ328_MODE;
			}else if (bl_lvl > 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP30_BL328_MODE;
			}
		} else if (temp == 35) {
			if (0x38 > bl_lvl && bl_lvl >= 0x0F) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL0F_MODE;
			} else if (0x7C > bl_lvl && bl_lvl >= 0x38) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL38_MODE;
			} else if (0x105 > bl_lvl && bl_lvl >= 0x7C) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL7C_MODE;
			} else if (0x217 > bl_lvl && bl_lvl >= 0x105) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL105_MODE;
			} else if (0x328 >= bl_lvl && bl_lvl >= 0x217) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL217_MODE;
			} else if (bl_lvl == 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BLEQ328_MODE;
			}else if (bl_lvl > 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP35_BL328_MODE;
			}
		} else if (temp == 45) {
			if (0x38 > bl_lvl && bl_lvl >= 0x0F) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL0F_MODE;
			} else if (0x7C > bl_lvl && bl_lvl >= 0x38) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL38_MODE;
			} else if (0x105 > bl_lvl && bl_lvl >= 0x7C) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL7C_MODE;
			} else if (0x217 > bl_lvl && bl_lvl >= 0x105) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL105_MODE;
			} else if (0x328 >= bl_lvl && bl_lvl >= 0x217) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL217_MODE;
			} else if (bl_lvl == 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BLEQ328_MODE;
			}else if (bl_lvl > 0x328) {
				dbi_bwg_type = DSI_CMD_SET_MI_DBI_BWG_TEMP45_BL328_MODE;
			}
	    }
	}

	if (mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_SA) {
		if (temp == 0){
	...
	if ((dbi_bwg_type != mi_cfg->dbi_bwg_type) && (dbi_bwg_type != DSI_CMD_SET_MAX)) {
		rc = dsi_panel_tx_cmd_set(panel, dbi_bwg_type,false);
		if (rc)
		{
...
int mi_dsi_panel_set_count_info(struct dsi_panel * panel, struct disp_count_info *count_info)
{
	...
	case DISP_COUNT_INFO_SWITCH_KERNEL_FUNCTION_TIMER:
		...
		break;
	case DISP_COUNT_INFO_TRUE_TONE_STATE:
		break;
	default:
		break;
	}
...
int mi_dsi_panel_dbv_demura_compensate(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
	mi_cfg = &panel->mi_cfg;
	mi_cfg->demura_cmd_executed = false;

	if (0 < bl_lvl && bl_lvl <= 206 && mi_cfg->demura_dc != STATE_ON) {
		mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_MI_PWM_DEMURA_SWITCH, bl_lvl);
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_PWM_DEMURA_SWITCH, false);
		if (rc == 0) {
			mi_cfg->demura_cmd_executed = true;
			mi_cfg->demura_dc = STATE_ON;
		}
    } else if (bl_lvl > 206 && mi_cfg->demura_dc != STATE_OFF) {
		mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_MI_DC_DEMURA_SWITCH, bl_lvl);
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DC_DEMURA_SWITCH, false);
		if (rc == 0) {
			mi_cfg->demura_cmd_executed = true;
			mi_cfg->demura_dc = STATE_OFF;
        }
    }
	return rc;
}
...
void mi_dsi_panel_update_ic_dimming_by_bl(struct dsi_panel *panel, u32 bl_lvl) {
	if(mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == O8_PANEL_SA ||
		...
		if(bl_lvl == 0 && (panel->mi_cfg.last_no_zero_bl_level > panel->bl_config.bl_normal_max) && dimming_on)
		{
			dimming_on = false;
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DIMMINGON, false);
			DISP_INFO("O8/P11 open dimming when HBM OFF,last_bl_level = %d bl_lvl = %d bl_normal_max = %d\n",
				panel->mi_cfg.last_no_zero_bl_level, bl_lvl, panel->bl_config.bl_normal_max);
		}
		if(bl_lvl && !dimming_on)
			dimming_on = true;
	}
...
ssize_t mi_dsi_panel_peakoffset_info_read(struct dsi_panel *panel)
{
	...
	int i, j;
	int16_t sum;
	void *peakoffset_ptr = NULL;
	static bool update_peak_offset = false;
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
	...
	if(update_peak_offset) {
		DISP_INFO(" peak offset has been updated !\n");
		return 0;
	}

	mi_cfg = &panel->mi_cfg;

	peakoffset_ptr = qcom_smem_get(QCOM_SMEM_HOST_ANY,
			SMEM_SW_DISPLAY_PEAK_OFFSET_TABLE, &item_size);
	if (IS_ERR(peakoffset_ptr) || item_size < 648) {
		DISP_ERROR("peakoffset read failed, size=%lu!\n", item_size);
		return -EINVAL;
	}

	DISP_INFO("peakoffset data size= %lu\n", item_size);
	/* Split raw smem data into 60/120/144/165Hz groups
	* Each group: 9 registers * 18 bytes = 162 bytes
	*/
	memcpy(mi_cfg->peak_offset_off_120hz, peakoffset_ptr, 162);
	memcpy(mi_cfg->peak_offset_off_144hz, peakoffset_ptr + 162, 162);
	memcpy(mi_cfg->peak_offset_off_165hz, peakoffset_ptr + 324, 162);
	memcpy(mi_cfg->peak_offset_off_60hz, peakoffset_ptr + 486, 162);

	/* Convert every 2 bytes to int16_t, add default, then split back to 2 bytes
	* off[i][j*2]=high byte, off[i][j*2+1]=low byte -> int16_t value
	* value + default -> high byte to on[i][j*2], low byte to on[i][j*2+1]
	*/
	for (i = 0; i < 9; i++) {
		for (j = 0; j < 9; j++) {
			sum = (int16_t)((mi_cfg->peak_offset_off_60hz[i][j * 2] << 8) |
				mi_cfg->peak_offset_off_60hz[i][j * 2 + 1])
					+ peak_offset_default_60hz[i][j];
				mi_cfg->peak_offset_on_60hz[i][j * 2] = (unsigned char)((sum >> 8) & 0xFF);
				mi_cfg->peak_offset_on_60hz[i][j * 2 + 1] = (unsigned char)(sum & 0xFF);

			sum = (int16_t)((mi_cfg->peak_offset_off_120hz[i][j * 2] << 8) |
				mi_cfg->peak_offset_off_120hz[i][j * 2 + 1])
					+ peak_offset_default_120hz[i][j];
				mi_cfg->peak_offset_on_120hz[i][j * 2] = (unsigned char)((sum >> 8) & 0xFF);
				mi_cfg->peak_offset_on_120hz[i][j * 2 + 1] = (unsigned char)(sum & 0xFF);

			sum = (int16_t)((mi_cfg->peak_offset_off_144hz[i][j * 2] << 8) |
				mi_cfg->peak_offset_off_144hz[i][j * 2 + 1])
					+ peak_offset_default_144hz[i][j];
				mi_cfg->peak_offset_on_144hz[i][j * 2] = (unsigned char)((sum >> 8) & 0xFF);
				mi_cfg->peak_offset_on_144hz[i][j * 2 + 1] = (unsigned char)(sum & 0xFF);

			sum = (int16_t)((mi_cfg->peak_offset_off_165hz[i][j * 2] << 8) |
				mi_cfg->peak_offset_off_165hz[i][j * 2 + 1])
					+ peak_offset_default_165hz[i][j];
				mi_cfg->peak_offset_on_165hz[i][j * 2] = (unsigned char)((sum >> 8) & 0xFF);
				mi_cfg->peak_offset_on_165hz[i][j * 2 + 1] = (unsigned char)(sum & 0xFF);
		}
	}
	update_peak_offset = true;
