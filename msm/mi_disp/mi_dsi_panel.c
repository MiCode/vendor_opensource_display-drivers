...
static int mi_dsi_send_lhbm_sync_with_te_locked(struct dsi_panel *panel,
			struct disp_feature_ctl *ctl)
{
	int rc = 0;
		...
		enum dsi_cmd_set_type type, int bl_lvl, bool lhbm_is_0size)
{
	...
	DISP_INFO("[%s] bl_lvl = %d, alpha = 0x%x%x type=%d\n",
		...
		u8 *update_val, u8 size , int bl_lvl, int type_index)
{
	...
	for(i = 0; i < sizeof(lhbm_coefficient_backlight_range)-1; i++) {
		...
		struct disp_feature_ctl *ctl)
{
	...
	case LOCAL_HBM_OFF_TO_NORMAL:
		...
		if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
			...
			lhbm_is_0size = true;
			if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
				DISP_DEBUG("LOCAL_HBM_NORMAL_WHITE_1000NIT in HBM\n");
				...
				if (is_aod_and_panel_initialized(panel)) {
					...
					case DOZE_BRIGHTNESS_HBM:
						update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
						DISP_DEBUG("LOCAL_HBM_NORMAL_WHITE_1000NIT in doze_hbm_dbv_level\n");
						break;
					case DOZE_BRIGHTNESS_LBM:
						update_bl_lvl = mi_cfg->doze_lbm_dbv_level;
						DISP_DEBUG("LOCAL_HBM_NORMAL_WHITE_1000NIT in doze_lbm_dbv_level\n");
						break;
					default:
						update_bl_lvl = mi_cfg->doze_hbm_dbv_level;
						DISP_DEBUG("LOCAL_HBM_NORMAL_WHITE_1000NIT use doze_hbm_dbv_level as defaults\n");
						break;
					}
				} else {
					update_bl_lvl = mi_cfg->last_bl_level;
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		rc = dsi_panel_tx_cmd_set(panel, lhbm_type);
		break;
	case LOCAL_HBM_NORMAL_WHITE_1000NIT_0SIZE:
		mi_cfg->dimming_state = STATE_DIM_BLOCK;
		lhbm_type = DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT;
		lhbm_is_0size = true;
		if (mi_cfg->last_bl_level > mi_cfg->lhbm_hbm_mode_threshold) {
