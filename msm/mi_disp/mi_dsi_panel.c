...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_DBI:
		...
		if (mi_cfg->feature_val[DISP_FEATURE_DBI] &&
			(mi_cfg->feature_val[DISP_FEATURE_DBI] != mi_cfg->real_dbi_state)) {
			if ((mi_get_panel_id_by_dsi_panel(panel) == O2_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == O11U_PANEL_PA) &&
				(panel->id_config.build_id >= PANEL_P20)) {
...
int dsi_panel_parse_osc_status_read_config(struct dsi_panel *panel)
{
	...
	osc_status_config = &panel->osc_status_config;
	if (!osc_status_config)
		return -EINVAL;

	dsi_panel_parse_cmd_sets_sub(&osc_status_config->osc_status_cmd,
			DSI_CMD_SET_MI_PANEL_OSC_STATUS_READ, utils);
	if (!osc_status_config->osc_status_cmd.count) {
		DISP_ERROR("osc_status_info read command parsing failed\n");
		return -EINVAL;
	}

	dsi_panel_parse_cmd_sets_sub(&osc_status_config->pre_tx_cmd,
			DSI_CMD_SET_MI_PANEL_OSC_STATUS_READ_PRE_TX, utils);
	if (!osc_status_config->pre_tx_cmd.count)
		DISP_INFO("cell_id_info pre command parsing failed\n");

	rc = utils->read_u32(utils->data, "mi,mdss-dsi-panel-osc-status-read-length",
				&osc_status_config->osc_status_cmds_rlen);
	if (rc || !osc_status_config->osc_status_cmds_rlen) {
		osc_status_config->osc_status_cmds_rlen = 0;
		return -EINVAL;
	}

	rc = utils->read_u32(utils->data, "mi,mdss-dsi-panel-osc-status-read-index",
				&osc_status_config->osc_status_read_info_index);
	if (rc || !osc_status_config->osc_status_read_info_index) {
		osc_status_config->osc_status_read_info_index = 0;
	}

	osc_status_config->return_buf = kcalloc(osc_status_config->osc_status_cmds_rlen,
		sizeof(unsigned char), GFP_KERNEL);
	if (!osc_status_config->return_buf) {
		return -ENOMEM;
	}
	return 0;
}
...
int mi_dsi_panel_update_mafr_status(struct dsi_panel *panel, struct dsi_rect *roi)
{
	...
	u8 mi_mafr_cfg[9][14] = {
		...
		{0x01,0x03,0x01,0x01,0x01,0x00,0x01,0x01,0x00,0x01,0x01,0x00,0x01,0x0B},
		/*index=4  idle 1HZ */
		{0x01,0x01,0x06,0x01,0x03,0x14,0x01,0x0B,0x16,0x01,0x01,0x00,0x01,0x77},
...
static u8 o3_dbv_param[2] = {0x03, 0xFF};
int mi_dsi_panel_auto_limit_by_dbv(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (bl_lvl >= 6143) {
		if ((o3_dbv_param[0] == ((O3_dbv_set[8191 - bl_lvl] >> 8) & 0xFF)) &&
			(o3_dbv_param[1] == (O3_dbv_set[8191 - bl_lvl] & 0xFF)))
			return 0;
		o3_dbv_param[0] = (O3_dbv_set[8191 - bl_lvl] >> 8) & 0xFF;
		o3_dbv_param[1] = O3_dbv_set[8191 - bl_lvl] & 0xFF;
		DISP_DEBUG("o3_dbv_param[1] = 0x%02X, o3_dbv_param[2] = 0x%02X",o3_dbv_param[0],o3_dbv_param[1]);
	} else {
		if (o3_dbv_param[0] == 0x03 && o3_dbv_param[1] == 0xFF)
			return 0;
		o3_dbv_param[0] = 0x03;
		o3_dbv_param[1] = 0xFF;
		DISP_DEBUG("o3_dbv_param[1] = 0x%02X, o3_dbv_param[2] = 0x%02X",o3_dbv_param[0],o3_dbv_param[1]);
		...
		if (info) {
			...
			if(info->mipi_address == 0xC1) {
				mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
						DSI_CMD_SET_MI_AUTO_LIMIT, info, o3_dbv_param, sizeof(o3_dbv_param));
			}
			info++;
		}
	}
