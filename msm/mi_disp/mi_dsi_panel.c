...
int mi_dsi_panel_csc_by_temper_comp_locked(struct dsi_panel *panel, int temp_val)
{
	...
	} else if (mi_get_panel_id_by_dsi_panel(panel) == O2_PANEL_PA && temp_val >= 40 && temp_val < 45) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_CSC_BY_TEMPER_COMP_40_MODE, false);
	} else if (mi_get_panel_id_by_dsi_panel(panel) == O2_PANEL_PA && temp_val >= 45) {
			...
			u32 fp_unlock_value)
{
	...
	if ((panel->power_mode == SDE_MODE_DPMS_LP1 ||
		...
		} else if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == O2_PANEL_PA) {
...
static u8 o3_dbv_param[2] = {0x03, 0x34};
