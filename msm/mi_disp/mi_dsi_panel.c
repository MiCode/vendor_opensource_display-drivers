...
static int mi_dsi_panel_update_low_temp_param_N2(struct dsi_panel *panel, int low_temp_index, enum dsi_cmd_set_type type) {
	int rc = 0;
	struct dsi_display_mode *cur_mode = NULL;
	struct dsi_cmd_update_info *info = NULL;
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
	...
	u32 cmd_update_index = 0;
	u32 cmd_update_count = 0;
	u8 dbi_81_reg[2] = {0x03, 0x32};

	if (mi_get_panel_id_by_dsi_panel(panel) != N2_PANEL_PA) {
		DISP_DEBUG("don't need update low temp param\n");
		return 0;
	}
	...
	cur_mode = panel->cur_mode;
	mi_cfg = &panel->mi_cfg;
	cmd_update_index = DSI_CMD_SET_LOW_TMP_ON_UPDATE;
	dbi_81_reg[1] = dbi_81_reg[1] + low_temp_index - 40;
	DISP_DEBUG("dbi_81_reg: %d ,low_temp_index : %d,type = %d\n", dbi_81_reg[1], low_temp_index, type);
...
int mi_dsi_panel_set_dbi_by_temp_N2_locked(struct dsi_panel *panel,
			int temp_val)
{
	int rc = 0;
	int real_temp_val = temp_val;
	int dbi_temp_index = 0;
	struct mi_dsi_panel_cfg *mi_cfg  = NULL;
	...
	if (real_temp_val >= N2_TMP_COMPENSATE_THRESHOLD && mi_cfg->low_tmp_state) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_LOW_TMP_OFF);
		mi_cfg->low_tmp_state = false;
		DISP_DEBUG("N2 low_tmp_state = false\n");
	} else if (real_temp_val < N2_TMP_COMPENSATE_THRESHOLD) {
		dbi_temp_index = real_temp_val/1000 + 20;
		if (dbi_temp_index < 0 ) dbi_temp_index = 0;
		else if (dbi_temp_index > 40) dbi_temp_index = 40;
		if (mi_cfg->low_tmp_index != dbi_temp_index) {
			mi_cfg->low_tmp_index = dbi_temp_index;
			mi_dsi_panel_update_low_temp_param_N2(panel,mi_cfg->low_tmp_index,DSI_CMD_SET_LOW_TMP_ON);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_LOW_TMP_ON);
			mi_cfg->low_tmp_state = true;
		}
	}
...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_DBI:
		if (mi_get_panel_id_by_dsi_panel(panel) == N11U_PANEL_PA) {
			...
			mi_dsi_panel_set_dbi_by_temp_N2_locked(panel, mi_cfg->feature_val[DISP_FEATURE_DBI]);
		} else {
