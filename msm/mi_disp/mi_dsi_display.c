			...
			u32 fp_unlock_value)
{
	...
	return mi_dsi_panel_set_fp_unlock_state(dsi_display->panel,
				fp_unlock_value);
}
...
int mi_dsi_display_set_te_width(void *display, u32 enable)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;

	if (!dsi_display && dsi_display->panel) {
		DISP_ERROR("Invalid display ptr and panel\n");
		return -EINVAL;
	}

	if (enable) {
		dsi_display->panel->te_width_status = 1;
	} else {
		dsi_display->panel->te_width_status = 0;
	}
	return 0;
}

int mi_dsi_display_get_te_width(void *display)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;
	...
	return dsi_display->panel->te_width_status;
}
