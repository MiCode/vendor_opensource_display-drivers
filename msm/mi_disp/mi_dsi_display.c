			...
			struct disp_feature_ctl *ctl)
{
	...
	if (sde_kms && mi_dsi_panel_need_tx_or_rx_cmd(ctl->feature_id)) {
		sde_vm_lock(sde_kms);
                        ...
                        char *buf, size_t size)
{
	...
	if(strlen(build_id_info_str)){
		...
		real_build_id = mi_get_panel_build_id(panel->mi_cfg.mi_panel_id, panel->id_config.build_id);
...
static int mi_dsi_display_re_read_wp_info(struct dsi_display *display)
{
	...
	if (cmd_sets->count != 0) {
		...
		if (rc) {
			DISP_ERROR("send wp_info cmd failed!");
	...
	rc = mi_dsi_display_cmd_read_panel_locked(display, cmd, config->return_buf,
                                                  config->wp_cmds_rlen);
	if (rc <= 0)
		DISP_ERROR("[DSI] Display command receive failed, rc=%d\n", rc);
	dsi_panel_release_panel_lock(display->panel);
...
int mi_dsi_display_cmd_read_panel_locked(struct dsi_display *display,
					  struct dsi_cmd_desc cmd, u8 *rx_buf, u32 rx_len)
{
	...
	rc = dsi_display_cmd_rx_panel_locked(display, &cmd);
	if (rc <= 0) {
		DISP_ERROR("[DSI] Display command receive failed, rc=%d\n", rc);
	}
	return rc;
...
static void mi_display_pm_suspend_delayed_work_handler(struct kthread_work *work)
{
	...
	if (dsi_panel->panel_initialized && pmic_pwrkey_status == PMIC_PWRKEY_BARK_TRIGGER) {
		...
		if(mi_get_panel_id(mi_cfg->mi_panel_id) == P82_PANEL_PA || mi_get_panel_id(mi_cfg->mi_panel_id) == P82_PANEL_PB ||
		   mi_get_panel_id(mi_cfg->mi_panel_id) == P81_PANEL_PA || mi_get_panel_id(mi_cfg->mi_panel_id) == P81_PANEL_PB){
			rc = dsi_panel_tx_cmd_set(dsi_panel, DSI_CMD_SET_OFF, false);
			if (rc) {
				pr_warn_ratelimited("[%s] failed to send DSI_CMD_SET_OFF cmds, rc=%d\n",
						dsi_panel->name, rc);
			}
			mi_cfg->tddi_gesture_flag = false;
			rc = dsi_panel_power_off(dsi_panel);
			if (rc) {
