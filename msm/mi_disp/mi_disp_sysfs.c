...
static ssize_t te_width_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	int ret = 0;
	unsigned long te_width;

	if (dd_ptr->intf_type == MI_INTF_DSI) {
		ret = kstrtoul(buf, 0, &te_width);
		if (ret)
			return ret;
		ret = mi_dsi_display_set_te_width(dd_ptr->display, te_width);
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
		ret = -EINVAL;
...
static ssize_t te_width_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	int ret = 0;
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return snprintf(buf, PAGE_SIZE, "%d\n", mi_dsi_display_get_te_width(dd_ptr->display));
	} else {
		snprintf(buf, PAGE_SIZE, "Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
		ret = -EINVAL;
...
static DEVICE_ATTR_RW(te_width);
