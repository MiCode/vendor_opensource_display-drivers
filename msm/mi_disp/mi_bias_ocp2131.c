...
#define OCP2131_Set_voltage	0x13
#define OCP2131_Set_vsp_vsn_enable	0x03
#define OCP2131_Set_value	0x5b
...
static int __ocp2131_write_reg(struct ocp2131 *ocp, int reg, u8 val)
{
	...
	if (ret < 0) {
		pr_err("hyper i2c write fail: can't write 0x%02X to reg 0x%02X: %d\n",
				val, reg, ret);
		pm_relax(ocp->dev);
		return ret;
	}
...
static int ocp2131_write_reg(struct ocp2131 *ocp, u8 reg, u8 data)
{
	...
	if (ret)
		pr_err("hyper Failed: reg=%02X, ret=%d\n", reg, ret);
	return ret;
}
/*
...
static void ocp2131_parse_dt(struct device *dev, struct ocp2131 *ocp)
{
	...
	ocp->cs_gpio = of_get_named_gpio(np, "bias,lcm-cs-gpio", 0);
	pr_info("cs: %d\n", ocp->cs_gpio);

	ocp->iovdd_gpio = of_get_named_gpio(np, "bias,lcm-iovdd-gpio", 0);
	pr_info("iovdd_gpio: %d\n", ocp->iovdd_gpio);

	ocp->enp_gpio = of_get_named_gpio(np, "bias,lcm-enp-gpio", 0);
	pr_info("enp_gpio: %d\n", ocp->enp_gpio);

	ocp->enn_gpio = of_get_named_gpio(np, "bias,lcm-enn-gpio", 0);
	pr_info("enn_gpio: %d\n", ocp->enn_gpio);

	if (gpio_is_valid(ocp->cs_gpio)) {
		ret = gpio_request(ocp->cs_gpio, "lcm-cs-gpio");
		if (ret < 0) {
			pr_err("failed to request lcm-cs-gpio\n");
		}
		pr_info("cs is valid!\n");
	}
  
	if (gpio_is_valid(ocp->iovdd_gpio)) {
		ret = gpio_request(ocp->iovdd_gpio, "lcm-iovdd-gpio");
		if (ret < 0) {
			pr_err("failed to request lcm-iovdd-gpio\n");
		}
		pr_info("iovdd_gpio is valid!\n");
	}
...
void bias_enable(void)
{
	struct ocp2131 *ocp = i2c_get_clientdata(gclient);

	pr_err("%s: entry\n", __func__);
	gpio_direction_output(ocp->cs_gpio, 1);
	udelay(5000);
	gpio_direction_output(ocp->iovdd_gpio, 1);
	udelay(5000);
	gpio_direction_output(ocp->enp_gpio, 1);
	udelay(5000);
	gpio_direction_output(ocp->enn_gpio, 1);


}

void bias_disable(void)
{
	struct ocp2131 *ocp = i2c_get_clientdata(gclient);

	pr_err("%s: entry\n", __func__);
	udelay(5000);
	gpio_direction_output(ocp->enn_gpio, 0);
	udelay(5000);
	gpio_direction_output(ocp->enp_gpio, 0);
	udelay(5000);
	gpio_direction_output(ocp->iovdd_gpio, 0);
	udelay(5000);
	gpio_direction_output(ocp->cs_gpio, 0);
}

static int ocp2131_bias_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct ocp2131 *ocp;
	pr_err("hyper Enter ocp2131_bias_probe\n");
...
int mi_bias_ocp2131_init(void)
{
	return i2c_add_driver(&ocp2131_bias_driver);
}
