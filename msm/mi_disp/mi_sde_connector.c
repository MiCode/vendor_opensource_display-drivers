...
#if defined(CONFIG_VIS_DISPLAY) || defined(CONFIG_VIS_DISPLAY_V2_D2)
			if (is_mi_dev_support_nova()) {
				vis_mi_esd_recover_handle_notify(mi_esd_err_irq_handle,display->panel->mi_cfg.esd_err_irq,c_conn);
			}
#endif
		}

		if (mi_get_panel_id_by_dsi_panel(display->panel) == P82_PANEL_PA ||
			mi_get_panel_id_by_dsi_panel(display->panel) == P82_PANEL_PB ||
			mi_get_panel_id_by_dsi_panel(display->panel) == P81_PANEL_PA ||
			mi_get_panel_id_by_dsi_panel(display->panel) == P81_PANEL_PB) {
...
int mi_sde_connector_send_emv_queue_work(struct drm_connector *connector)
{
	...
	if (!df) {
		DISP_ERROR("mi_get_disp_feature failed\n");
		return -EINVAL;
	}
...
int mi_sde_connector_install_properties(struct sde_connector *c_conn)
{
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		msm_property_install_range(&c_conn->property_info, "mi_mafr_info",
			0x0, 0, U32_MAX, 0, CONNECTOR_PROP_MI_MAFR_INFO);
