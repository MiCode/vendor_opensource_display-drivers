...
#define SMEM_SW_DISPLAY_PANEL_TYPE_TABLE 501
...
int mi_dsi_panel_parse_esd_gpio_config(struct dsi_panel *panel)
{
	...
	mi_cfg->esd_err_irq_gpio_second = of_get_named_gpio(
			utils->data, "mi,esd-err-irq-gpio-second", 0);
	...
	rc = utils->read_u32(utils->data, "mi,esd-err-irq-gpio-flag-second", &mi_cfg->esd_err_irq_flags_second);
	if (mi_cfg->esd_err_irq_flags_second)
			DISP_DEBUG("mi,esd-err-irq-gpio-flag-second is defined\n");
...
static int mi_dsi_panel_parse_dc_config(struct dsi_panel *panel)
{
	...
	} else if (!strcmp(string, "type_dc_pwm_switch_skip_bl")) {
		mi_cfg->dc_type = TYPE_DC_PWM_SWITCH_SKIP_BL;
	} else {
...
int mi_dsi_panel_parse_config(struct dsi_panel *panel)
{
	...
	mi_cfg->panel_forbid_fps_switch_in_aod = utils->read_bool(utils->data, "mi,panel-forbid-fps-switch-in-aod");
	if (mi_cfg->panel_forbid_fps_switch_in_aod)
		pr_info("panel forbid fps switch in aod\n");

	panel_type_ptr = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_SW_DISPLAY_PANEL_TYPE_TABLE, &item_size);
	if (!IS_ERR(panel_type_ptr) && item_size > 0) {
		memcpy(&mi_cfg->panel_type, panel_type_ptr, item_size);
		DSI_INFO("panel type is %d\n", mi_cfg->panel_type);
	}
