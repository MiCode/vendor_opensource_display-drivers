		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		...
		if (mi_get_panel_id_by_dsi_panel(display->panel) == N16T_PANEL_PA ||
			...
			mi_get_panel_id_by_dsi_panel(display->panel) == O16U_PANEL_PC ||
		...
		if (mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PA ||
			mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PC ||
			mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PB) {
	...
	for (mode_idx = 0; mode_idx < timing_mode_count; mode_idx++) {
		...
		for (i = 0; i < num_dfps_rates; i++) {
			...
			if (mi_get_panel_id_by_dsi_panel(display->panel) == N16T_PANEL_PA ||
				...
				mi_get_panel_id_by_dsi_panel(display->panel) == O16U_PANEL_PC ||
				mi_get_panel_id_by_dsi_panel(display->panel) == O16U_PANEL_PB ||
				mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PA ||
				mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PC ||
				mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PB) {
...
#ifdef MI_DISPLAY_MODIFY
			/*Override VFP  in P16U_PANEL_PA && PB*/
			if(mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PA ||
				mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PC ||
				mi_get_panel_id_by_dsi_panel(display->panel) == P16U_PANEL_PB) {
				if(sub_mode->timing.refresh_rate == 90) {
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->panel->mi_cfg.feature_val[DISP_FEATURE_DBI] &&
		...
		else if(mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PA || mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PB || mi_get_panel_id_by_dsi_panel(panel) == P16U_PANEL_PC)
			mi_dsi_panel_csc_by_temper_comp(panel, panel->mi_cfg.feature_val[DISP_FEATURE_DBI]);
		else
