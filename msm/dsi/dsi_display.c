...
int dsi_host_transfer_sub(struct mipi_dsi_host *host, struct dsi_cmd_desc *cmd)
{
	...
	//mi_dsi_print_log(display->panel, cmd);
	dsi_display_set_cmd_tx_ctrl_flags(display, cmd);
			 ...
			 u32 flags)
{
	...
	DSI_INFO("mdp_transfer_time=%d, hactive=%d, vactive=%d, fps=%d, clk_rate=%llu,"
			"v_front_porch=%d, v_back_porch=%d, h_front_porch=%d, h_back_porch=%d,"
			"ddic_mode=%s, sf_fps=%d, ddic_min_fps=%d\n",
			adj_mode.priv_info->mdp_transfer_time_us,
			timing.h_active, timing.v_active, timing.refresh_rate,
			adj_mode.priv_info->clk_rate_hz,
			timing.v_front_porch, timing.v_back_porch, timing.h_front_porch, timing.h_back_porch,
