...
/* Single ESD register check item */
struct esd_register_check {
    const u8 *pre_cmd;        /* DSI command packet */
    size_t pre_cmd_len;       /* command length */
    const u8 *read_cmd;        /* DSI command packet */
    size_t read_cmd_len;       /* command length */
    u8 reg;               /* register address (for logging) */
    size_t rdlen;         /* expected read length */
};
/* ESD configuration for each panel */
struct esd_panel_config {
    const char *panel_name;           /* Used to match panel ->name */
    struct esd_register_check esd_checks[5]; /* Supports up to 5 register checks */
};
/* Pre defined DSI commands (reusable) */
static const u8 cmd_0a[] = {0x06, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0A};
static const u8 cmd_ab[] = {0x06, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0xAB};
static const u8 cmd_f0[] = {0x39, 0x01, 0x00, 0x00, 0x00, 0x00, 0x04, 0xF0, 0x99, 0x51, 0x0C};
static const u8 cmd_d3[] = {0x06, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0xD3};
// You can continue to add: cmd_xx[], cmd_yy[], ...
static struct esd_panel_config esd_configs[] = {
    {
        .panel_name = "p85 36 02 0b wqxga dsc vid panel",
        .esd_checks = {
            { .read_cmd = cmd_0a, .read_cmd_len = sizeof(cmd_0a), .reg = 0x0A, .rdlen = 1 },
            { .read_cmd = cmd_ab, .read_cmd_len = sizeof(cmd_ab), .reg = 0xAB, .rdlen = 2 },
            { } // end marker (required)
        }
    },
    {
        .panel_name = "p85 42 0f 0a wqxga dsc vid panel",
        .esd_checks = {
            { .read_cmd = cmd_0a, .read_cmd_len = sizeof(cmd_0a), .reg = 0x0A, .rdlen = 1 },
            { .pre_cmd = cmd_f0, .pre_cmd_len = sizeof(cmd_f0), .read_cmd = cmd_d3, .read_cmd_len = sizeof(cmd_d3), .reg = 0xD3, .rdlen = 2 },
            { }
        }
    },
};
#define ESD_CONFIG_COUNT ARRAY_SIZE(esd_configs)
static int read_dsi_reg(struct dsi_display *display,
                        const struct esd_register_check *item)
{
    int rc;
    int i;
    u8 rdbuf[20];
    if (!display || !item->read_cmd)
        return -EINVAL;
    /* Step 1: Send pre-command (e.g., page switch) if required */
    if (item->pre_cmd && item->pre_cmd_len > 0) {
        rc = dsi_display_cmd_transfer(display->drm_conn, display, (u8 *)item->pre_cmd, item->pre_cmd_len);
        if (rc < 0) {
            DSI_ERR("[ESD] Pre-cmd for reg 0x%02x failed, rc=%d\n", item->reg, rc);
            return rc;
        }
    }
    /* Step 2: Read the register */
    memset(rdbuf, 0, sizeof(rdbuf));
    rc = dsi_display_cmd_receive(display, (u8 *)item->read_cmd, item->read_cmd_len, rdbuf, item->rdlen);
    if (rc <= 0) {
        DSI_ERR("[ESD] Read reg 0x%02x FAILED, rc=%d\n", item->reg, rc);
        return rc;
    }
    /* Step 3: Log the read result */
    DSI_ERR("[ESD] Reg 0x%02x =", item->reg);
    for (i = 0; i < item->rdlen; i++)
        DSI_ERR(" 0x%02x", rdbuf[i]);
    DSI_ERR("\n");
    return rc;
}
int read_esd_reg(struct dsi_display *display)
{
    int rc = 0;
    int i;
    int esd_fail_count = 0;
    const struct esd_panel_config *config = NULL;
    if (!display || !display->panel) {
        DSI_ERR("[ESD] Invalid display/panel pointer\n");
        return -EINVAL;
    }
    if (sde_kms_is_suspend_blocked(display->drm_dev)) {
        DSI_ERR("[ESD] SDE KMS suspended, skip ESD check\n");
        return -EBUSY;
    }
    DSI_ERR("[ESD] Starting ESD check for panel: %s\n", display->panel->name);
    /* Find matching ESD configuration */
    for (i = 0; i < ESD_CONFIG_COUNT; i++) {
        if (strncmp(display->panel->name, esd_configs[i].panel_name,
                    strlen(esd_configs[i].panel_name)) == 0) {
            config = &esd_configs[i];
            break;
        }
    }
    if (!config) {
        DSI_ERR("[ESD] No ESD config found for panel: %s\n", display->panel->name);
        return 0; // No config, not an error
    }
    /* Acquire panel lock to prevent race conditions */
    //dsi_panel_acquire_panel_lock(display->panel);
    /* Execute each check in the configuration */
    for (i = 0; config->esd_checks[i].read_cmd; i++) {
        rc = read_dsi_reg(display, &config->esd_checks[i]);
        if (rc <= 0) {
            esd_fail_count++;
            // Continue with other checks for maximum information
        }
    }
    //dsi_panel_release_panel_lock(display->panel);
    /* Final result */
    if (esd_fail_count > 0) {
        DSI_ERR("[ESD] ESD check FAILED (%d errors)\n", esd_fail_count);
        return -EIO; // Trigger recovery (e.g., panel reset)
    }
    DSI_ERR("[ESD] ESD read reg completed\n");
    return 0;
}
