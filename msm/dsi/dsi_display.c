				...
				u32 curr_refresh_rate)
{
	...
	if(1){
		if (timing->refresh_rate == 120) {
			...
			adj_mode->timing.h_front_porch = 30;
			adj_mode->timing.h_back_porch = 20;
		} else if (timing->refresh_rate == 90) {
			adj_mode->timing.v_front_porch = 26;
			adj_mode->timing.h_front_porch = 124;
			adj_mode->timing.h_back_porch = 124;
		} else if (timing->refresh_rate == 60) {
			adj_mode->timing.v_front_porch = 2784;
			adj_mode->timing.h_front_porch = 30;
			adj_mode->timing.h_back_porch = 20;
		} else if (timing->refresh_rate == 50) {
			adj_mode->timing.v_front_porch = 2232;
			adj_mode->timing.h_front_porch = 124;
			adj_mode->timing.h_back_porch = 124;
		} else if (timing->refresh_rate == 48) {
			adj_mode->timing.v_front_porch = 2439;
			adj_mode->timing.h_front_porch = 124;
			adj_mode->timing.h_back_porch = 124;
		} else if (timing->refresh_rate == 30) {
			adj_mode->timing.v_front_porch = 5542;
			adj_mode->timing.h_front_porch = 124;
			adj_mode->timing.h_back_porch = 124;
		}
	}else{ 
...
int mi_display_powerkey_callback(int status)
{
	...
	if(status == PMIC_PWRKEY_CLOSE_BRIGHNESS){
		...
		rc = gpio_direction_output(panel->bl_config.en_gpio, 0);
		if (rc)
...
extern volatile bool is_shutdown;
...
int dsi_display_unprepare(struct dsi_display *display)
{
	...
	DSI_INFO("[%s] enter\n",__func__);
	DSI_INFO("[%s]\n",__func__,is_skip_op_required(display));
...
module_param_string(dsi_display_whitepoint, whitepoint_info_s, MAX_CMDLINE_PARAM_LEN, 0600);
MODULE_PARM_DESC(whitepoint, "display whitepoint info");
