...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (true == get_lcd_reset_gpio_keep_high()) {
		if(is_shutdown==false) {
			DSI_INFO("[TP gesture], lcd_reset_keep_high\n");
        	return rc;
		} else {
			DSI_INFO("[%s], shutdown\n",__func__);
		}
	}else {
        DSI_INFO("[TP gesture], lcd_reset_keep_low\n");
    }
	...
	if (gpio_is_valid(panel->reset_config.reset_gpio) && !panel->reset_gpio_always_on) {
		...
		if(is_shutdown==false) {
			...
			DSI_INFO("[%s], shutdown, reset set low\n",__func__);
			gpio_set_value(panel->reset_config.reset_gpio, 0);
		}
	}
...
static int dsi_panel_parse_dfps_caps(struct dsi_panel *panel)
{
	...
	supported = false;
	DSI_ERR("[%s] DISPLAY_FACTORY_BUILD defined\n", name);
#endif
	if (!supported) {
