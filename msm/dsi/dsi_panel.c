...
extern bool lcd_bkl_pwm_set;

static bool lcd_esd_check_status = false;
int get_lcd_esd_check_status(void)
{
	return lcd_esd_check_status;
}
EXPORT_SYMBOL(get_lcd_esd_check_status);

void set_lcd_esd_check_status(bool en)
{
	lcd_esd_check_status = en;
}
EXPORT_SYMBOL(set_lcd_esd_check_status);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	lcd_esd_check_status = panel->mi_cfg.panel_dead_flag;

	if (true == get_lct_tp_gesture_status() && panel->mi_cfg.panel_dead_flag == false) {
		DSI_INFO("[TP gesture], lcd_bias_keep_hight\n");
		return rc;
	} else if (panel->mi_cfg.panel_dead_flag == true) {
		DSI_INFO("ESD occurred, power off\n");
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_EXTERNAL:
		rc = sy7758_backlight_update_status(bl_lvl);
		if (!rc && !lcd_bkl_pwm_set) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_BACKLIGHT_PWM);
			if (rc) {
				DSI_ERR("[%s] failed to send DSI_CMD_SET_BACKLIGHT_PWM cmds, rc=%d\n", panel->name, rc);
			} else {
				lcd_bkl_pwm_set = true;
				DSI_INFO("[backlight], set lcd bkl pwm \n");
			}
		}
		break;
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-gesture-commands",
	"mi,mdss-dsi-backlight-pwm-command",
#endif
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-gesture-commands-state",
	"mi,mdss-dsi-backlight-pwm-command-state",
#endif
};
...
#endif
		if (true == get_lct_tp_gesture_status() && panel->mi_cfg.panel_dead_flag == false) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_GESTURE);
		} else {
