...
#define	MIN_REG_VALUE_HX	7	//华星最低寄存器亮度值
#define NORMAL_MAX_REG_HX	1642	//华星noraml段最高寄存器亮度值
#define HBM_MAX_REG_HX		2011	//华星hbm段最高寄存器亮度值

#define	MIN_REG_VALUE_XL	7	//信利最低寄存器亮度值
#define NORMAL_MAX_REG_XL	1643	//信利noraml段最高寄存器亮度值
#define HBM_MAX_REG_XL		2011	//信利hbm段最高寄存器亮度值
	...
	u32 bl_lvl)
{
	...
	if ((panel->bl_config.bl_lcd_number) == 2) {
		if(bl_lvl <= 0){
			...
			bl_lvl = (bl_lvl - MIN_BL) * (NORMAL_MAX_REG_XL - MIN_REG_VALUE_XL) / (NORMAL_MAX_BL - MIN_BL) + MIN_REG_VALUE_XL;
			pr_info("%s:TM_hbm_off,bl_lvl= %d\n",__func__,bl_lvl);
		} else if (bl_lvl <= MAX_BL) {
			bl_lvl = (bl_lvl - NORMAL_MAX_BL) * (HBM_MAX_REG_XL - NORMAL_MAX_REG_XL) / (MAX_BL - NORMAL_MAX_BL) + NORMAL_MAX_REG_XL;
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(esd_config->esd_err_irq_gpio)) {
		...
		if (rc)
			pr_err("%s: [LC_DEBUG] Failed to get esd irq gpio %d (code: %d)",
