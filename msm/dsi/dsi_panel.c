...
#endif
{
	...
	DSI_INFO("+type=%d state=%d count=%d\n", type, state, count);
	for (i = 0; i < count; i++) {
		cmds->ctrl_flags = 0;
...
#define UHBM_THRESHOLD    3127
#define UHBM_MAX          4095
#define APL_MIN           33
#define APL_MAX           136
#define DBV_RANGE         (UHBM_MAX - UHBM_THRESHOLD)
#define APL_RANGE         (APL_MIN - APL_MAX)

static u8 calculate_apl_gain(int bl_lvl) {
    int diff = bl_lvl - UHBM_THRESHOLD;
    return (u8)((diff * APL_RANGE) / DBV_RANGE + APL_MAX);
}

static int mi_dsi_update_uhbm_cmd_cc_reg(struct dsi_panel *panel,
			enum dsi_cmd_set_type type, int bl_lvl)
{
	...
	DSI_INFO("Entering UHBM mode for DBV=%d\n", bl_lvl);

	u8 uhbm_cmd_cc_reg[3] = {0x01, 0x00, calculate_apl_gain(bl_lvl)};
	DSI_INFO("APL_Gain calculated: %d\n", calculate_apl_gain(bl_lvl));
	...
	struct dsi_cmd_update_info *info = priv_info->cmd_update[cmd_update_index];
	u32 cmd_update_count = priv_info->cmd_update_count[cmd_update_index];

	for (int j = 0; j < cmd_update_count; j++) {
		...
		if (info->mipi_address == 0xCC) {
			mi_dsi_panel_update_cmd_set(panel, panel->cur_mode,
					type, info, uhbm_cmd_cc_reg, sizeof(uhbm_cmd_cc_reg));
			break;
		} else {
	...
	DSI_INFO("UHBM ON success\n");
	return 0;
}

static int dsi_panel_exit_uhbm_mode(struct dsi_panel *panel)
{
	DSI_INFO("Exiting UHBM mode\n");
	int rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_UHBM_EXIT);
	if (rc)
		DSI_ERR("DSI_CMD_SET_UHBM_EXIT failed\n");
	else
		DSI_INFO("UHBM OFF success\n");
	return rc;
}
...
int dsi_panel_update_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	int rc = 0;
	const int disp_id = MI_DISP_PRIMARY;
	static bool uhbm_active = false;
	static bool uhbm_53_state = true;
	const u8 uhbm_53_enter[] = {0x53, 0xE8};
	const u8 uhbm_53_exit[]  = {0x53, 0xE0};
	...
	if (panel->cur_mode->timing.refresh_rate == 30) {
		panel->mi_cfg.aod_to_normal_bl_delay = true;
		panel->mi_cfg.aod_to_normal_bl = bl_lvl;
		DSI_INFO("Currently in the AOD scenario, delay the brightness distribution of the system\n");
		return rc;
	}
	panel->mi_cfg.aod_to_normal_bl_delay = false;
	mi_disp_feature_event_notify_by_type(disp_id,
			MI_DISP_EVENT_BACKLIGHT,
			sizeof(bl_lvl),
			bl_lvl);
	DSI_INFO("%s: bl_lvl=%d %llu\n", __func__, bl_lvl, panel->mi_count.panel_id);


	bool need_uhbm = (bl_lvl >= UHBM_THRESHOLD);
	u32 uhbm_bl_lvl = bl_lvl;

	if (panel->bl_config.bl_inverted_dbv)
		bl_lvl = (((bl_lvl & 0xff) << 8) | (bl_lvl >> 8));

	if (panel->mi_count.panel_id == 2 || panel->mi_count.panel_id == 3) {
		if (need_uhbm) {
			// 首次进入UHBM时发送0x53命令
			if (uhbm_53_state) {
				rc = mipi_dsi_generic_write(dsi, uhbm_53_enter, sizeof(uhbm_53_enter));
				if (rc >= 0)
					uhbm_53_state = false;
				else
					DSI_ERR("UHBM 53 set E8 failed, rc=%d\n", rc);
			}
			mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_UHBM_ENTER, uhbm_bl_lvl);
			mi_dsi_update_uhbm_cmd_cc_reg(panel, DSI_CMD_SET_UHBM_ENTER, uhbm_bl_lvl);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_UHBM_ENTER);
			if (rc)
				DSI_ERR("UHBM enter failed, rc=%d\n", rc);
			else
				uhbm_active = true;
		} else {
			// 退出UHBM时发送0x53命令
			if (!uhbm_53_state) {
				rc = mipi_dsi_generic_write(dsi, uhbm_53_exit, sizeof(uhbm_53_exit));
				if (rc >= 0)
					uhbm_53_state = true;
				else
					DSI_ERR("UHBM 53 set 00 failed, rc=%d\n", rc);
			}
			rc = mipi_dsi_dcs_set_display_brightness(dsi, bl_lvl);
			if (rc < 0)
				DSI_ERR("failed to update dcs backlight: %d\n", bl_lvl);
			if (uhbm_active) {
				rc = dsi_panel_exit_uhbm_mode(panel);
				if (rc)
					DSI_ERR("UHBM exit failed, rc=%d\n", rc);
				else
					uhbm_active = false;
			}
		}
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-doze-hbm-exit-command",
	"mi,mdss-dsi-doze-lbm-exit-command",
	...
	"mi,mdss-dsi-local-hbm-white-gamma-pre-read-command",
	...
	"mi,mdss-dsi-uhbm-enter-command",
	"mi,mdss-dsi-uhbm-exit-command",
	"mi,mdss-dsi-doze-hbm-exit-60hz-command",
	"mi,mdss-dsi-doze-lbm-exit-60hz-command",
#endif
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-dispparam-fps-90hz-command-state",
	...
	"qcom,mdss-dsi-dispparam-hbm-on-l4-command-state",
	"qcom,mdss-dsi-dispparam-hbm-on-l5-command-state",
	...
	"mi,mdss-dsi-dispparam-dimming-hbmon-command-state",
	"mi,mdss-dsi-dispparam-dimming-hbmoff-command-state",
	...
	"mi,mdss-dsi-doze-hbm-exit-command-state",
	"mi,mdss-dsi-doze-lbm-exit-command-state",
	...
	"mi,mdss-dsi-local-hbm-110nit-off-to-normal-command-state",
	...
	"mi,mdss-dsi-local-hbm-white-gamma-pre-read-command-state",
	...
	"mi,mdss-dsi-uhbm-enter-command-state",
	"mi,mdss-dsi-uhbm-exit-command-state",
	"mi,mdss-dsi-doze-hbm-exit-60hz-command-state ",
	"mi,mdss-dsi-doze-lbm-exit-60hz-command-state",
#endif
};
...
const char *cmd_set_update_map[DSI_CMD_UPDATE_MAX] = {
	...
	"mi,mdss-dsi-uhbm-enter-command-update",
#endif
};
...
static void dsi_match_panel_name_id(struct dsi_panel *panel)
{
	...
	DSI_DEBUG("panel name is: %s,please check!\n", panel->name);
}
			...
			int topology_override)
{
	...
	dsi_for_each_child_node(timings_np, child_np) {
		...
		if (rc) {
			DSI_ERR("failed to parse fps params, rc=%d\n", rc);
		}
...
error:
	...
	printk("LC_DEBUG func:%s , line= %d \n", __func__, __LINE__);
		if (rc) {
			DSI_ERR("[%s] failed to update fps code setting, rc=%d\n",
					panel->name, rc);
		}
...
int dsi_panel_video_mode_pre_aod_locked(struct dsi_panel *panel)
{
	...
	if (panel->cur_mode->timing.refresh_rate != 30 &&
		...
		if (!panel->mi_cfg.need_aod_exit_set_backlight) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_AOD_EXIT);
		} else {
			DSI_INFO("last_doze_brightness :%d \n", panel->mi_cfg.last_doze_brightness);
			if ((panel->cur_mode->timing.refresh_rate == 60) &&
			    ((!strcmp(panel->name, "dsi p17 35 0F 0b amoled video mode dsi panel with DSC"))
			    || (!strcmp(panel->name, "dsi p17 41 0F 0c amoled video mode dsi panel with DSC")))) {
				DSI_INFO("this is 0b/0c panel, and fps is 60hz\n");
				if (panel->mi_cfg.last_doze_brightness == DOZE_BRIGHTNESS_HBM) {
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DOZE_HBM_EXIT_60HZ);
					if (rc) {
						DISP_ERROR("[%s] failed to send DSI_CMD_SET_MI_DOZE_HBM_EXIT_60HZ cmd, rc=%d\n",
						panel->type, rc);
					}
				} else if (panel->mi_cfg.last_doze_brightness == DOZE_BRIGHTNESS_LBM) {
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DOZE_LBM_EXIT_60HZ);
					if (rc) {
						DISP_ERROR("[%s] failed to send DSI_CMD_SET_MI_DOZE_LBM_EXIT_60HZ cmd, rc=%d\n",
						panel->type, rc);
					}
				}
			} else {
				if (panel->mi_cfg.last_doze_brightness == DOZE_BRIGHTNESS_HBM) {;
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DOZE_HBM_EXIT);
					if (rc) {
						DISP_ERROR("[%s] failed to send DSI_CMD_SET_MI_DOZE_HBM_EXIT cmd, rc=%d\n",
						panel->type, rc);
					}
				} else if (panel->mi_cfg.last_doze_brightness == DOZE_BRIGHTNESS_LBM) {
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_DOZE_LBM_EXIT);
					if (rc) {
						DISP_ERROR("[%s] failed to send DSI_CMD_SET_MI_DOZE_LBM_EXIT cmd, rc=%d\n",
						panel->type, rc);
					}
				}
			}
		}
		if (panel->mi_cfg.aod_to_normal_bl_delay && panel->mi_cfg.last_doze_brightness != DOZE_TO_NORMAL) {
			dsi_panel_update_backlight(panel, panel->mi_cfg.aod_to_normal_bl);
			panel->mi_cfg.aod_to_normal_bl_delay = false;
			DSI_INFO("AOD has been turned off, sending system brightness\n");
		}
		panel->mi_cfg.aod_exit_flags = true;
		panel->mi_cfg.last_doze_brightness = DOZE_TO_NORMAL;
...
extern int pre_doze_brightness;
int dsi_panel_video_mode_post_aod_locked(struct dsi_panel *panel)
{
	...
	mi_cfg = &panel->mi_cfg;
	int doze_brightness=mi_cfg->doze_brightness;
	...
	if (panel->cur_mode->timing.refresh_rate == 30 &&
		...
		if (!mi_cfg->need_aod_enter_set_backlight) {
			...
			doze_brightness = (doze_brightness == DOZE_TO_NORMAL && panel->mi_cfg.last_doze_brightness == DOZE_TO_NORMAL) ? pre_doze_brightness : doze_brightness;
			if (doze_brightness == DOZE_BRIGHTNESS_HBM) {
					panel->mi_cfg.panel_state = PANEL_STATE_DOZE_HIGH;
					panel->mi_cfg.last_doze_brightness = doze_brightness;
		...
		DSI_INFO("doze_brightness :%d,pre_doze_brightness :%d\n", doze_brightness, pre_doze_brightness);
