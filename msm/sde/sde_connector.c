...
#ifdef MI_DISPLAY_MODIFY
	mi_sde_connector_ip_N2(connector);
#endif
...
#endif
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		if (display->panel && !display->panel->mi_cfg.cur_lhbm_on_state && display->panel->mi_cfg.lhbm_off_sync_te)
			mi_sde_connector_lhbm_on_fence(connector);
	}
