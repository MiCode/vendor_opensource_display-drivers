		...
		void *arg)
{
	...
	for (reg_idx = 0; reg_idx < intr->sde_irq_size; reg_idx++) {
		...
		if (start_idx >= intr->sde_irq_map_size ||
			...
			SDE_EVT32(reg_idx, start_idx, end_idx);
			continue;
		}
		...
		if (irq_status) {
			SDE_REG_WRITE(&intr->hw, intr->sde_irq_tbl[reg_idx].clr_off, irq_status);
			SDE_EVT32(irq_status, enable_mask, intr->sde_irq_tbl[reg_idx].status_off);
		}
