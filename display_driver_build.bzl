...
def define_target_variant_modules(target, variant, registry, modules, config_options = [], device = None, override_hdrs = None):
    if not device:
        kernel_build = "{}_{}".format(target, variant)
    else:
        kernel_build = "{}_{}".format(device, variant)
    kernel_build_label = "//msm-kernel:{}".format("{}_{}".format(target, variant))
    modules = [registry.get(module_name) for module_name in modules]
    options = _get_kernel_build_options(modules, config_options)
    build_print = lambda message : print("{}: {}".format(kernel_build, message))
    formatter = lambda s : s.replace("%b", "{}_{}".format(target, variant)).replace("%t", target)
    if override_hdrs:
        headers = ["//msm-kernel:all_headers"] + override_hdrs
    else:
