...
#include "drm_dp_mst_topology_ser.h"
...
void
mi_drm_dp_encode_sideband_req(const struct drm_dp_sideband_msg_req_body *req,
...
EXPORT_SYMBOL_FOR_TESTS_ONLY(mi_drm_dp_encode_sideband_req);

/* Decode a sideband request we've encoded, mainly used for debugging */
int
mi_drm_dp_decode_sideband_req(const struct drm_dp_sideband_msg_tx *raw,
...
EXPORT_SYMBOL_FOR_TESTS_ONLY(mi_drm_dp_decode_sideband_req);

void
mi_drm_dp_dump_sideband_msg_req_body(const struct drm_dp_sideband_msg_req_body *req,
...
EXPORT_SYMBOL_FOR_TESTS_ONLY(mi_drm_dp_dump_sideband_msg_req_body);
				...
				const struct drm_dp_sideband_msg_tx *txmsg)
{
	...
	ret = mi_drm_dp_decode_sideband_req(txmsg, &req);
	if (ret) {
		drm_printf(p, "<failed to decode sideband req: %d>\n", ret);
		return;
	}
	mi_drm_dp_dump_sideband_msg_req_body(&req, 1, p);
			     ...
			     u8 port_num, u32 offset, u8 num_bytes, u8 *bytes)
{
	...
	mi_drm_dp_encode_sideband_req(&req, msg);
}
...
#if defined(CONFIG_MI_DISP_AUTO)
	unsigned long wait_timeout = msecs_to_jiffies(4);
#else
...
static void drm_dp_free_mst_branch_device(struct kref *kref)
{
	...
	if (mstb->port_parent)
		mi_drm_dp_mst_put_port_malloc(mstb->port_parent);
...
/**
 * mi_drm_dp_mst_get_port_malloc() - Increment the malloc refcount of an MST port
 ...
 *
 * See also: mi_drm_dp_mst_put_port_malloc()
 */
void
mi_drm_dp_mst_get_port_malloc(struct drm_dp_mst_port *port)
{
	kref_get(&port->malloc_kref);
	drm_dbg(port->mgr->dev, "port %p (%d)\n", port, kref_read(&port->malloc_kref));
}
EXPORT_SYMBOL(mi_drm_dp_mst_get_port_malloc);

/**
 * mi_drm_dp_mst_put_port_malloc() - Decrement the malloc refcount of an MST port
 ...
 * no longer be used.
 *
 * See also: mi_drm_dp_mst_get_port_malloc()
 */
void
mi_drm_dp_mst_put_port_malloc(struct drm_dp_mst_port *port)
{
	drm_dbg(port->mgr->dev, "port %p (%d)\n", port, kref_read(&port->malloc_kref) - 1);
	kref_put(&port->malloc_kref, drm_dp_free_mst_port);
}
EXPORT_SYMBOL(mi_drm_dp_mst_put_port_malloc);
...
EXPORT_SYMBOL(mi_drm_atomic_get_mst_payload_state);
...
static void drm_dp_destroy_port(struct kref *kref)
{
	...
	if (port->input) {
		mi_drm_dp_mst_put_port_malloc(port);
		return;
	}
		    ...
		    bool new_mcs)
{
	...
	if (port->pdt != DP_PEER_DEVICE_NONE) {
		if (drm_dp_mst_is_end_device(port->pdt, port->mcs)) {
...
#if defined(CONFIG_MI_DISP_AUTO)
			//Add this condition, this part will never be executed
			 ...
			 */
			mi_drm_dp_mst_get_port_malloc(port);
			mutex_unlock(&mgr->lock);
...
#if 0
/**
 * mi_drm_dp_mst_connector_late_register() - Late MST connector registration
...
EXPORT_SYMBOL(mi_drm_dp_mst_connector_late_register);

/**
 * mi_drm_dp_mst_connector_early_unregister() - Early MST connector unregistration
 * @connector: The MST connector
 * @port: The MST port for this connector
 *
 * Helper to unregister the remote aux device for this MST port, registered by
 * mi_drm_dp_mst_connector_late_register(). Drivers should call this from their mst
...
EXPORT_SYMBOL(mi_drm_dp_mst_connector_early_unregister);
#endif
...
static void drm_dp_mst_link_probe_work(struct work_struct *work)
{
	 ...
	 * mi_drm_dp_mst_topology_mgr_set_mst(). Namely, the CableMatters USB-C
...
#if defined(CONFIG_MI_DISP_AUTO)
static int drm_dp_init_dummy_reply(struct drm_dp_link_address_ack_reply *reply){
	int i = 0;
	...
	reply->nports= 2;
	for (i = 0;i<2;i++){
		/*if this port is an input port,default is false*/
		reply->ports[i].input_port = false;
		/*peer_device_type: DP_PEER_DEVICE_SST_SINK*/
		reply->ports[i].peer_device_type = 0x3;
		reply->ports[i].port_number = 1;
		/*message capability status is false*/
		reply->ports[i].mcs = false;
		/*displayport device plug status is true*/
		reply->ports[i].ddps = true;
		/*legacy device plug status is false*/
		reply->ports[i].legacy_device_plug_status = false;
		reply->ports[i].dpcd_revision = 0x14;
		/*num_sdp_streams and num_sdp_stream_sinks are none*/
		reply->ports[i].num_sdp_streams = 0;
		reply->ports[i].num_sdp_stream_sinks = 0;
	}
	/*fixed port numer: 0 and 1*/
	reply->ports[1].port_number = 0;
	/*peer_guid is none*/
	memset(reply->ports[0].peer_guid,0,16);
	memset(reply->ports[1].peer_guid,0,16);
	return 0;
}

static int drm_dp_send_link_address_dummy_for_serder(struct drm_dp_mst_topology_mgr *mgr,
				     struct drm_dp_mst_branch *mstb){
	struct drm_dp_link_address_ack_reply reply;
	...
	list_for_each_entry_safe(port, tmp, &mstb->ports, next) {
		...
		drm_err(mgr->dev, "port %d was not in link address, removing\n",
			    port->port_num);
		list_del(&port->next);
		drm_dp_mst_topology_put_port(port);
		changed = true;
				     ...
				     struct drm_dp_mst_branch *mstb)
{
	...
	bool changed = false;
#if defined(CONFIG_MI_DISP_AUTO)
	return drm_dp_send_link_address_dummy_for_serder(mgr,mstb);
#endif
...
#if defined(CONFIG_MI_DISP_AUTO)
static int
drm_dp_send_enum_path_resources_dummy_for_serder(struct drm_dp_mst_topology_mgr *mgr,
				struct drm_dp_mst_branch *mstb,
				struct drm_dp_mst_port *port)
{
	/*just return a default value for full pbn*/
	port->full_pbn =30000;
	/*fec_capable is false*/
	port->fec_capable = false;
...
#if defined(CONFIG_MI_DISP_AUTO)
	return drm_dp_send_enum_path_resources_dummy_for_serder(mgr,mstb,port);
#endif
	txmsg = kzalloc(sizeof(*txmsg), GFP_KERNEL);
	if (!txmsg)
		return -ENOMEM;
				   ...
				   int pbn)
{
	...
	int i;
#if defined(CONFIG_MI_DISP_AUTO)
	//just return, do nothing for Serializer
	return 0;
#endif
...
EXPORT_SYMBOL(mi_drm_dp_send_power_updown_phy);
		...
		struct drm_dp_query_stream_enc_status_ack_reply *status)
{
	...
	drm_modeset_lock(&mgr->base.lock, NULL);
	state = mi_to_drm_dp_mst_topology_state(mgr->base.state);
	payload = mi_drm_atomic_get_mst_payload_state(state, port);
...
EXPORT_SYMBOL(mi_drm_dp_send_query_stream_enc_status);
...
/**
 * mi_drm_dp_add_payload_part1() - Execute payload update part 1
 * @mgr: Manager to use.
...
EXPORT_SYMBOL(mi_drm_dp_add_payload_part1);

/**
 * mi_drm_dp_remove_payload() - Remove an MST payload
 * @mgr: Manager to use.
 * @mst_state: The MST atomic state
			   ...
			   struct drm_dp_mst_atomic_payload *new_payload)
{
	...
	if (new_payload->delete)
		mi_drm_dp_mst_put_port_malloc(new_payload->port);
}
EXPORT_SYMBOL(mi_drm_dp_remove_payload);

/**
 * mi_drm_dp_add_payload_part2() - Execute payload update part 2
 * @mgr: Manager to use.
 * @state: The global atomic state
 * @payload: The payload to update
 *
 * If @payload was successfully assigned a starting time slot by mi_drm_dp_add_payload_part1(), this
...
EXPORT_SYMBOL(mi_drm_dp_add_payload_part2);
...
/**
 * mi_drm_dp_get_vc_payload_bw - get the VC payload BW for an MST link
...
EXPORT_SYMBOL(mi_drm_dp_get_vc_payload_bw);

/**
 * mi_drm_dp_read_mst_cap() - check whether or not a sink supports MST
 * @aux: The DP AUX channel to use
 * @dpcd: A cached copy of the DPCD capabilities for this sink
 *
 * Returns: %True if the sink supports MST, %false otherwise
 */
bool mi_drm_dp_read_mst_cap(struct drm_dp_aux *aux,
...
EXPORT_SYMBOL(mi_drm_dp_read_mst_cap);

/**
 * mi_drm_dp_mst_topology_mgr_set_mst() - Set the MST state for a topology manager
...
EXPORT_SYMBOL(mi_drm_dp_mst_topology_mgr_set_mst);
...
/**
 * mi_drm_dp_mst_topology_mgr_suspend() - suspend the MST manager
...
EXPORT_SYMBOL(mi_drm_dp_mst_topology_mgr_suspend);

/**
 * mi_drm_dp_mst_topology_mgr_resume() - resume the MST manager
 ...
 * otherwise.
 */
int mi_drm_dp_mst_topology_mgr_resume(struct drm_dp_mst_topology_mgr *mgr,
				   bool sync)
{
	int ret;
	u8 guid[16];
...
EXPORT_SYMBOL(mi_drm_dp_mst_topology_mgr_resume);
...
/**
 * mi_drm_dp_mst_hpd_irq() - MST hotplug IRQ notify
...
EXPORT_SYMBOL(mi_drm_dp_mst_hpd_irq);

/**
 * mi_drm_dp_mst_detect_port() - get connection status for an MST port
...
EXPORT_SYMBOL(mi_drm_dp_mst_detect_port);

/**
 * mi_drm_dp_mst_get_edid() - get EDID for an MST port
...
EXPORT_SYMBOL(mi_drm_dp_mst_get_edid);

/**
 * mi_drm_dp_atomic_find_time_slots() - Find and add time slots to the state
 ...
 * restraints of @mgr until the driver calls mi_drm_dp_mst_atomic_check().
 *
 * Additionally, it is OK to call this function multiple times on the same
 * @port as needed. It is not OK however, to call this function and
 * mi_drm_dp_atomic_release_time_slots() in the same atomic check phase.
 *
 * See also:
 * mi_drm_dp_atomic_release_time_slots()
 * mi_drm_dp_mst_atomic_check()
 *
 * Returns:
 * Total slots in the atomic state assigned for this port, or a negative error
 * code if the port no longer exists
 */
int mi_drm_dp_atomic_find_time_slots(struct drm_atomic_state *state,
				  struct drm_dp_mst_topology_mgr *mgr,
				  struct drm_dp_mst_port *port, int pbn)
{
	...
	topology_state = mi_drm_atomic_get_mst_topology_state(state, mgr);
	...
	payload = mi_drm_atomic_get_mst_payload_state(topology_state, port);
	if (payload) {
...
EXPORT_SYMBOL(mi_drm_dp_atomic_find_time_slots);

/**
 * mi_drm_dp_atomic_release_time_slots() - Release allocated time slots
 * @state: global atomic state
 * @mgr: MST topology manager for the port
 ...
 * Additionally, it is OK to call this function multiple times on the same
 * @port as needed. It is not OK however, to call this function and
 * mi_drm_dp_atomic_find_time_slots() on the same @port in a single atomic check
 * phase.
 *
 * See also:
 * mi_drm_dp_atomic_find_time_slots()
 * mi_drm_dp_mst_atomic_check()
 *
 * Returns:
 * 0 on success, negative error code otherwise
 */
int mi_drm_dp_atomic_release_time_slots(struct drm_atomic_state *state,
				     struct drm_dp_mst_topology_mgr *mgr,
				     struct drm_dp_mst_port *port)
{
	struct drm_dp_mst_topology_state *topology_state;
...
EXPORT_SYMBOL(mi_drm_dp_atomic_release_time_slots);

/**
 * mi_drm_dp_mst_atomic_setup_commit() - setup_commit hook for MST helpers
 * @state: global atomic state
 *
 ...
 */
int mi_drm_dp_mst_atomic_setup_commit(struct drm_atomic_state *state)
{
	struct drm_dp_mst_topology_mgr *mgr;
	struct drm_dp_mst_topology_state *mst_state;
	...
	mi_for_each_new_mst_mgr_in_state(state, mgr, mst_state, i) {
...
EXPORT_SYMBOL(mi_drm_dp_mst_atomic_setup_commit);

/**
 * mi_drm_dp_mst_atomic_wait_for_dependencies() - Wait for all pending commits on MST topologies,
...
void mi_drm_dp_mst_atomic_wait_for_dependencies(struct drm_atomic_state *state)
{
	...
	mi_for_each_oldnew_mst_mgr_in_state(state, mgr, old_mst_state, new_mst_state, i) {
		...
		list_for_each_entry(old_payload, &old_mst_state->payloads, next) {
			...
			new_payload = mi_drm_atomic_get_mst_payload_state(new_mst_state,
								       old_payload->port);
			new_payload->vc_start_slot = old_payload->vc_start_slot;
		}
	}
}
EXPORT_SYMBOL(mi_drm_dp_mst_atomic_wait_for_dependencies);

/**
 * mi_drm_dp_mst_root_conn_atomic_check() - Serialize CRTC commits on MST-capable connectors operating
 * in SST mode
 ...
 * 0 on success, negative error code otherwise
 */
int mi_drm_dp_mst_root_conn_atomic_check(struct drm_connector_state *new_conn_state,
				      struct drm_dp_mst_topology_mgr *mgr)
{
	...
	if (new_conn_state->crtc) {
		...
		if (crtc_state && drm_atomic_crtc_needs_modeset(crtc_state)) {
			mst_state = mi_drm_atomic_get_mst_topology_state(state, mgr);
			if (IS_ERR(mst_state))
...
EXPORT_SYMBOL(mi_drm_dp_mst_root_conn_atomic_check);

/**
 * mi_drm_dp_mst_update_slots() - updates the slot info depending on the DP ecoding format
...
EXPORT_SYMBOL(mi_drm_dp_mst_update_slots);
...
retry:
	...
	drm_dbg_kms(mgr->dev, "xxxxxxxxxxxx 1111 %x\n", status);
	if (ret < 0) {
...
/**
 * mi_drm_dp_check_act_status() - Polls for ACT handled status.
 * @mgr: manager to use
 *
...
EXPORT_SYMBOL(mi_drm_dp_check_act_status);

/**
 * mi_drm_dp_calc_pbn_mode() - Calculate the PBN for a mode.
...
EXPORT_SYMBOL(mi_drm_dp_calc_pbn_mode);
			       ...
			       int namelen)
{
	...
	mst_edid = mi_drm_dp_mst_get_edid(port->connector, mgr, port);
	drm_edid_get_monitor_name(mst_edid, name, namelen);
	kfree(mst_edid);
}

/**
 * mi_drm_dp_mst_dump_topology(): dump topology to seq file.
...
EXPORT_SYMBOL(mi_drm_dp_mst_dump_topology);
...
drm_dp_mst_duplicate_state(struct drm_private_obj *obj)
{
	...
	list_for_each_entry(pos, &old_state->payloads, next) {
		...
		mi_drm_dp_mst_get_port_malloc(payload->port);
...
fail:
	list_for_each_entry_safe(pos, payload, &state->payloads, next) {
		mi_drm_dp_mst_put_port_malloc(pos->port);
		kfree(pos);
	}
	kfree(state);
...
static int
mi_drm_dp_mst_atomic_check_port_bw_limit(struct drm_dp_mst_port *port,
				      struct drm_dp_mst_topology_state *state);

static int
mi_drm_dp_mst_atomic_check_mstb_bw_limit(struct drm_dp_mst_branch *mstb,
				      struct drm_dp_mst_topology_state *state)
{
	struct drm_dp_mst_atomic_payload *payload;
	struct drm_dp_mst_port *port;
	...
	list_for_each_entry(port, &mstb->ports, next) {
		ret = mi_drm_dp_mst_atomic_check_port_bw_limit(port, state);
		if (ret < 0)
			return ret;
				      ...
				      struct drm_dp_mst_topology_state *state)
{
	...
	if (drm_dp_mst_is_end_device(port->pdt, port->mcs)) {
		...
		pbn_used = mi_drm_dp_mst_atomic_check_mstb_bw_limit(port->mstb,
								 state);
		if (pbn_used <= 0)
			return pbn_used;
	}
...
static inline int
mi_drm_dp_mst_atomic_check_payload_alloc_limits(struct drm_dp_mst_topology_mgr *mgr,
					     struct drm_dp_mst_topology_state *mst_state)
{
	struct drm_dp_mst_atomic_payload *payload;
...
/**
 * mi_drm_dp_mst_add_affected_dsc_crtcs
 ...
 *
 * See also:
 * mi_drm_dp_mst_atomic_enable_dsc()
 */
int mi_drm_dp_mst_add_affected_dsc_crtcs(struct drm_atomic_state *state, struct drm_dp_mst_topology_mgr *mgr)
{
	struct drm_dp_mst_topology_state *mst_state;
	struct drm_dp_mst_atomic_payload *pos;
	...
	list_for_each_entry(pos, &mst_state->payloads, next) {
		...
		if (!mi_drm_dp_mst_dsc_aux_for_port(pos->port))
			continue;
...
EXPORT_SYMBOL(mi_drm_dp_mst_add_affected_dsc_crtcs);

/**
 * mi_drm_dp_mst_atomic_enable_dsc - Set DSC Enable Flag to On/Off
				 ...
				 int pbn, bool enable)
{
	...
	mst_state = mi_drm_atomic_get_mst_topology_state(state, port->mgr);
	if (IS_ERR(mst_state))
		return PTR_ERR(mst_state);

	payload = mi_drm_atomic_get_mst_payload_state(mst_state, port);
	if (!payload) {
	...
	if (enable) {
		time_slots = mi_drm_dp_atomic_find_time_slots(state, port->mgr, port, pbn);
		drm_dbg_atomic(state->dev,
...
EXPORT_SYMBOL(mi_drm_dp_mst_atomic_enable_dsc);

/**
 * mi_drm_dp_mst_atomic_check - Check that the new state of an MST topology in an
 ...
 */
int mi_drm_dp_mst_atomic_check(struct drm_atomic_state *state)
{
	struct drm_dp_mst_topology_mgr *mgr;
	struct drm_dp_mst_topology_state *mst_state;
	int i, ret = 0;
	...
	mi_for_each_new_mst_mgr_in_state(state, mgr, mst_state, i) {
		...
		ret = mi_drm_dp_mst_atomic_check_payload_alloc_limits(mgr, mst_state);
		if (ret)
			break;

		mutex_lock(&mgr->lock);
		ret = mi_drm_dp_mst_atomic_check_mstb_bw_limit(mgr->mst_primary,
							    mst_state);
		mutex_unlock(&mgr->lock);
		if (ret < 0)
			break;
...
EXPORT_SYMBOL(mi_drm_dp_mst_atomic_check);

const struct drm_private_state_funcs mi_drm_dp_mst_topology_state_funcs = {
	.atomic_duplicate_state = drm_dp_mst_duplicate_state,
	.atomic_destroy_state = drm_dp_mst_destroy_state,
};
EXPORT_SYMBOL(mi_drm_dp_mst_topology_state_funcs);

/**
 * mi_drm_atomic_get_mst_topology_state: get MST topology state
 * @state: global atomic state
 ...
 */
struct drm_dp_mst_topology_state *mi_drm_atomic_get_mst_topology_state(struct drm_atomic_state *state,
								    struct drm_dp_mst_topology_mgr *mgr)
{
	return to_dp_mst_topology_state(drm_atomic_get_private_obj_state(state, &mgr->base));
}
EXPORT_SYMBOL(mi_drm_atomic_get_mst_topology_state);

/**
 * mi_drm_atomic_get_old_mst_topology_state: get old MST topology state in atomic state, if any
 * @state: global atomic state
 * @mgr: MST topology manager, also the private object in this case
 *
...
EXPORT_SYMBOL(mi_drm_atomic_get_old_mst_topology_state);

/**
 * mi_drm_atomic_get_new_mst_topology_state: get new MST topology state in atomic state, if any
 * @state: global atomic state
 * @mgr: MST topology manager, also the private object in this case
 *
...
EXPORT_SYMBOL(mi_drm_atomic_get_new_mst_topology_state);

/**
 * mi_drm_dp_mst_topology_mgr_init - initialise a topology manager
				 ...
				 int conn_base_id)
{
	...
	drm_atomic_private_obj_init(dev, &mgr->base,
				    &mst_state->base,
				    &mi_drm_dp_mst_topology_state_funcs);

	return 0;
}
EXPORT_SYMBOL(mi_drm_dp_mst_topology_mgr_init);

/**
 * mi_drm_dp_mst_topology_mgr_destroy() - destroy topology manager.
 * @mgr: manager to destroy
 */
void mi_drm_dp_mst_topology_mgr_destroy(struct drm_dp_mst_topology_mgr *mgr)
{
	mi_drm_dp_mst_topology_mgr_set_mst(mgr, false);
	flush_work(&mgr->work);
...
EXPORT_SYMBOL(mi_drm_dp_mst_topology_mgr_destroy);
			       ...
			       struct i2c_msg *msgs, int num)
{
	...
	txmsg->dst = mstb;
	mi_drm_dp_encode_sideband_req(&msg, txmsg);
...
/**
 * mi_drm_dp_mst_dsc_aux_for_port() - Find the correct aux for DSC
...
EXPORT_SYMBOL(mi_drm_dp_mst_dsc_aux_for_port);
