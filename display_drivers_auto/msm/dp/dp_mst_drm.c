...
#if defined(CONFIG_MI_DISP_AUTO)
		status = mi_drm_dp_mst_detect_port(connector, ctx, mgr, port);
#else
...
#if defined(CONFIG_MI_DISP_AUTO)
	pbn = mi_drm_dp_calc_pbn_mode(pinfo->pixel_clk_khz, bpp, false);
#else
...
static const struct dp_drm_mst_fw_helper_ops drm_dp_mst_fw_helper_ops = {
	...
	.atomic_find_time_slots    = mi_drm_dp_atomic_find_time_slots,
	.update_payload_part1      = mi_drm_dp_add_payload_part1,
	.check_act_status          = mi_drm_dp_check_act_status,
	.update_payload_part2      = mi_drm_dp_add_payload_part2,
	.detect_port_ctx           = dp_mst_detect_port,
	.get_edid                  = mi_drm_dp_mst_get_edid,
	.topology_mgr_set_mst      = mi_drm_dp_mst_topology_mgr_set_mst,
	.get_vcpi_info             = _dp_mst_get_vcpi_info,
	.atomic_release_time_slots = mi_drm_dp_atomic_release_time_slots,
	.reset_vcpi_slots          = mi_drm_dp_remove_payload,
#else
	.calc_pbn_mode             = dp_mst_calc_pbn_mode,
...
#if (KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE)
#if defined(CONFIG_MI_DISP_AUTO)
	mst_state = mi_to_drm_dp_mst_topology_state(mst->mst_mgr.base.state);
#else
...
#if defined(CONFIG_MI_DISP_AUTO)
	rc = mi_drm_dp_mst_atomic_check(state);
#else
...
static int _dp_mst_bridge_pre_enable_part1(struct dp_mst_bridge *dp_bridge)
{
...
#if defined(CONFIG_MI_DISP_AUTO)
		mi_drm_dp_send_power_updown_phy(&mst->mst_mgr, port, true);
#else
	...
	mi_drm_dp_mst_update_slots(mst_state, DP_CAP_ANSI_8B10B);
#else
	mst_state = to_drm_dp_mst_topology_state(mst->mst_mgr.base.state);
	payload = drm_atomic_get_mst_payload_state(mst_state, port);
	if (!payload || payload->time_slots <= 0) {
...
static void _dp_mst_bridge_pre_disable_part2(struct dp_mst_bridge *dp_bridge)
{
	...
	if (mst->state == PM_SUSPEND) {
		dp_display->wakeup_phy_layer(dp_display, true);
#if defined(CONFIG_MI_DISP_AUTO)
		mi_drm_dp_send_power_updown_phy(&mst->mst_mgr, port, false);
#else
		...
		void *display, const struct msm_resource_caps_info *avail_res)
{
	...
	struct sde_connector *c_conn = to_sde_connector(connector);
	struct drm_dp_mst_port *mst_port = (struct drm_dp_mst_port *)c_conn->mst_port;
	...
	struct dp_panel_info fixed_info_cluster = {
		.h_active = 300,
		.v_active = 1390,
		.h_back_porch = 16,
		.h_front_porch = 69,
	...
	struct dp_panel_info fixed_info_hud = {
		.h_active = 1280,
		.v_active = 640,
		.h_back_porch = 40,
		.h_front_porch = 45,
	...
	DP_MST_DEBUG_V("enter:  port_num %d\n",mst_port->port_num);
	SDE_EVT32_EXTERNAL(SDE_EVTLOG_FUNC_ENTRY, connector->base.id);
	if (mst_port->port_num == port_hud){
		memcpy(&dp_mode.timing, &fixed_info_hud, sizeof(fixed_info_hud));
	}else if (mst_port->port_num == port_cluster){
		memcpy(&dp_mode.timing, &fixed_info_cluster, sizeof(fixed_info_cluster));
	}else{
		memcpy(&dp_mode.timing, &fixed_info_hud, sizeof(fixed_info_hud));
	}
	//if (!list_empty(&connector->modes)){
	//	DP_MST_DEBUG("have modes, do nothing\n");
	//	return 1;
	//}
...
#if defined(CONFIG_MI_DISP_AUTO)
	mi_drm_dp_mst_put_port_malloc(c_conn->mst_port);
#else
...
#if defined(CONFIG_MI_DISP_AUTO)
	mi_drm_dp_mst_get_port_malloc(c_conn->mst_port);
#else
...
#if defined(CONFIG_MI_DISP_AUTO)
	rc = mi_drm_dp_mst_hpd_irq(&mst->mst_mgr, esi, &handled);
#else
...
#else
#if defined(CONFIG_MI_DISP_AUTO)
	ret = mi_drm_dp_mst_topology_mgr_init(&dp_mst.mst_mgr, dev,
#else
	ret = drm_dp_mst_topology_mgr_init(&dp_mst.mst_mgr, dev,
#endif
					dp_mst.caps.drm_aux,
...
#if defined(CONFIG_MI_DISP_AUTO)
	mi_drm_dp_mst_topology_mgr_destroy(&mst->mst_mgr);
#else
