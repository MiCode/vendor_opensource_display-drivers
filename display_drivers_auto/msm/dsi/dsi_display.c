		...
		int power_mode, void *disp)
{
	...
	DISP_TIME_INFO("Display (%s), Power mode (%s)\n", display->display_type,
		mi_disp_get_power_mode_name(power_mode));
#endif
...
static int dsi_display_parse_dt(struct dsi_display *display)
{
	...
	display->max967xx_dev = max967xx_data_get(of_node);
#endif
	DSI_DEBUG("success\n");
error:
	return rc;
...
#if defined(CONFIG_MI_DISP_AUTO)
	if (!is_skip_op_required(display) && display->max967xx_dev)
		max967xx_set_power_mode(display->max967xx_dev, SERDES_STATE_OPEN);
#endif
	...
	if (!display->poms_pending && !is_skip_op_required(display)) {
		...
		if (rc)
			DSI_ERR("[%s] panel post-unprepare failed, rc=%d\n",
			       display->name, rc);
#if defined(CONFIG_MI_DISP_AUTO)
		if (display->max967xx_dev)
			max967xx_set_power_mode(display->max967xx_dev, SERDES_STATE_CLOSE);
#endif
	}
	display->hw_ownership = false;
