...
static int shd_display_bind(struct device *dev, struct device *master, void *data)
{
	...
	if (!dev || !pdev || !master) {
		SDE_ERROR("invalid param(s), dev %pK, pdev %pK, master %pK\n", dev, pdev, master);
		rc = -EINVAL;
		goto end;
	}
	...
	if (!drm || !shd_dev) {
		SDE_ERROR("invalid param(s), drm %pK, shd_dev %pK\n", drm, shd_dev);
		rc = -EINVAL;
		goto end;
	}
	...
	if (rc) {
		SDE_ERROR("failed to register component notifier\n");
		goto end;
	}
...
static int sde_shd_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		SDE_ERROR("component add failed, rc=%d\n", ret);
		goto error;
	}
